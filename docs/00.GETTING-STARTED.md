...
See also the related [README.md](https://github.com/LightSourceEngine/jjs/blob/master/jjs-core/profiles/README.md).
...
*Note*: The heap size will be allocated statically at compile time, when JJS memory
allocator is used.
...
```bash
python tools/run-tests.py --jjs-tests
```
