 ...
 */
void
jerry_init_ex (jerry_init_flag_t flags, /**< combination of Jerry flags */
               jerry_init_options_t* options) /**< extra init options */
{
  jerry_init_options_t clean_options = { .flags = JERRY_INIT_OPTION_ALL };
  uint32_t options_flags = options ? options->flags : JERRY_INIT_OPTION_NO_OPTS;

  if (options_flags & JERRY_INIT_OPTION_HAS_HEAP_SIZE)
  {
    clean_options.heap_size = options->heap_size;
  }
  else
  {
    clean_options.heap_size = JERRY_GLOBAL_HEAP_SIZE;
  }

  if (options_flags & JERRY_INIT_OPTION_HAS_GC_LIMIT)
  {
  ...
  if (options->gc_limit == 0)
  {
    clean_options.gc_limit = (clean_options.heap_size * 1024) / 32;
  }

  if (options_flags & JERRY_INIT_OPTION_HAS_GC_MARK_LIMIT)
  {
    clean_options.gc_mark_limit = options->gc_mark_limit;
  }
  else
  {
    clean_options.gc_mark_limit = JERRY_GC_MARK_LIMIT;
  }

  if (options_flags & JERRY_INIT_OPTION_HAS_STACK_LIMIT)
  {
    clean_options.stack_limit = options->stack_limit;
  }
  else
  {
    clean_options.stack_limit = JERRY_STACK_LIMIT;
  }

  if (options_flags & JERRY_INIT_OPTION_HAS_GC_NEW_OBJECTS_FRACTION)
  {
  ...
  size_t total_size = jerry_port_context_alloc (sizeof (jerry_context_t), &clean_options);
