...
/**
 * Type for directly encoded integer numbers in JJS.
 */
 ...
 */
#define ECMA_IS_VALUE_ERROR(value) (JJS_UNLIKELY ((value) == ECMA_VALUE_ERROR))
 ...
 */
struct jjs_call_info_t;

/**
 * Type of an external function handler.
 */
typedef ecma_value_t (*ecma_native_handler_t) (const struct jjs_call_info_t *call_info_p,
 ...
 * This pair is represented by a single pointer in JJS. Although
 ...
 *     and jjs_class_object_type must be updated as well
 */
typedef enum
{
...
/**
 * Offset for JJS_CONTEXT (status_flags) top 8 bits.
 */
#define ECMA_LOCAL_PARSE_OPTS_OFFSET ((sizeof (uint32_t) - sizeof (uint8_t)) * JJS_BITSINBYTE)
  ...
  union
  {
    ...
    struct
    {
      ...
      union
      {
        ...
        uint8_t error_type; /**< jjs_error_t type of native error objects */
#if JJS_BUILTIN_DATE
...
#define ECMA_NAME_DATA_PROPERTY_DESCRIPTOR_BITS                                                                    \
  ((uint16_t) (JJS_PROP_IS_VALUE_DEFINED | JJS_PROP_IS_CONFIGURABLE_DEFINED | JJS_PROP_IS_ENUMERABLE_DEFINED \
...
#define ECMA_PROPERTY_FLAGS_MASK \
  ((uint16_t) (JJS_PROP_IS_CONFIGURABLE | JJS_PROP_IS_ENUMERABLE | JJS_PROP_IS_WRITABLE))
...
/**
 * Flag that identifies that the string is static which means it is stored in JJS_CONTEXT (string_list_cp)
 */
