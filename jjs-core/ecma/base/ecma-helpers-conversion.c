 ...
 */
inline static int JJS_ATTR_CONST
 ...
 */
inline static int JJS_ATTR_ALWAYS_INLINE JJS_ATTR_CONST
...
{
  JJS_ASSERT (radix == 2 || radix == 8 || radix == 16);
  JJS_ASSERT (*str_p == LIT_CHAR_0);
                            ...
                            uint32_t options) /**< allowing underscore option bit */
{
  ...
  {
    JJS_ASSERT (ecma_uint64_clz (significand_uint128.hi) == 4);
  ...
  JJS_ASSERT (ecma_uint64_clz (significand_uint128.hi) == 11);
  ...
  JJS_ASSERT (binary_exponent < (1 << ECMA_NUMBER_BIASED_EXP_WIDTH) - 1);
  JJS_ASSERT (significand < (1ull << ECMA_NUMBER_FRACTION_WIDTH));
                            ...
                            lit_utf8_size_t buffer_size) /**< size of buffer */
{
  ...
  if (JJS_LIKELY (buf_p != out_buffer_p))
  {
...
{
  if (JJS_UNLIKELY (ecma_number_is_zero (num) || !ecma_number_is_finite (num)))
  {
    return 0;
  }
  ...
  JJS_ASSERT (num_in_uint32_range < uint64_2_pow_32);
  ...
  {
    JJS_ASSERT (ret == uint64_2_pow_32 - uint32_num);
  }
  else
  {
    JJS_ASSERT (ret == uint32_num);
  }
#endif /* !JJS_NDEBUG */
...
ecma_number_to_int32 (ecma_number_t num) /**< ecma-number */
{
  ...
  JJS_ASSERT (int64_num >= 0);

  if (int64_num >= uint32_2_pow_31)
  {
    JJS_ASSERT (ret == int64_num - int64_2_pow_32);
  }
  else
  {
    JJS_ASSERT (ret == int64_num);
  }
#endif /* !JJS_NDEBUG */
                        ...
                        int32_t *out_decimal_exp_p) /**< [out] decimal exponent */
{
  ...
  JJS_ASSERT (!ecma_number_is_zero (num));
  JJS_ASSERT (!ecma_number_is_infinity (num));
  JJS_ASSERT (!ecma_number_is_negative (num));
                            ...
                            lit_utf8_size_t buffer_size) /**< size of buffer */
{
  ...
  if (ecma_number_is_infinity (num))
  {
    ...
    JJS_ASSERT (dst_p <= buffer_p + buffer_size);
    return (lit_utf8_size_t) (dst_p - buffer_p);
  }
