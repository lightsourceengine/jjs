 ...
 */
bool
ecma_create_native_pointer_property (ecma_context_t *context_p, /**< JJS context */
                                     ecma_object_t *obj_p, /**< object to create property in */
                                     void *native_p, /**< native pointer */
                                     const jjs_object_native_info_t *native_info_p) /**< native type info */
{
  ...
  if (property_p == NULL)
  {
    native_pointer_p = (ecma_native_pointer_t *) jmem_heap_alloc_block (context_p, sizeof (ecma_native_pointer_t));

    ecma_property_value_t *value_p;
    ECMA_CREATE_INTERNAL_PROPERTY (context_p, obj_p, name_p, property_p, value_p);

    ECMA_SET_INTERNAL_VALUE_POINTER (context_p, value_p->value, native_pointer_p);
  ...
  else if (*property_p & ECMA_PROPERTY_FLAG_SINGLE_EXTERNAL)
  {
    ...
    native_pointer_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_native_pointer_t, value_p->value);
    ...
    *property_p &= (ecma_property_t) ~ECMA_PROPERTY_FLAG_SINGLE_EXTERNAL;
    ...
    item_p = (ecma_native_pointer_chain_t *) jmem_heap_alloc_block (context_p, sizeof (ecma_native_pointer_chain_t));
    item_p->data = *native_pointer_p;

    jmem_heap_free_block (context_p, native_pointer_p, sizeof (ecma_native_pointer_t));

    item_p->next_p = (ecma_native_pointer_chain_t *) jmem_heap_alloc_block (context_p, sizeof (ecma_native_pointer_chain_t));
    item_p->next_p->next_p = NULL;

    native_pointer_p = &item_p->next_p->data;
    ECMA_SET_INTERNAL_VALUE_POINTER (context_p, value_p->value, item_p);
  }
  else
  {
    ecma_property_value_t *value_p = ECMA_PROPERTY_VALUE_PTR (property_p);
    ...
    {
      ecma_native_pointer_chain_t *item_p;
      item_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_native_pointer_chain_t, value_p->value);

      /* There should be at least 2 native pointers in the chain */
      JJS_ASSERT (item_p != NULL && item_p->next_p != NULL);
      ...
      new_item_p = (ecma_native_pointer_chain_t *) jmem_heap_alloc_block (context_p, sizeof (ecma_native_pointer_chain_t));
...
ecma_get_native_pointer_value (ecma_context_t *context_p, /**< JJS context */
                               ecma_object_t *obj_p, /**< object to get property value from */
                               const jjs_object_native_info_t *native_info_p) /**< native type info */
{
  if (ecma_op_object_is_fast_array (obj_p))
  {
  ...
  if (JJS_LIKELY (*property_p & ECMA_PROPERTY_FLAG_SINGLE_EXTERNAL))
  {
    ecma_native_pointer_t *native_pointer_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_native_pointer_t, value_p->value);
 ...
 */
bool
ecma_delete_native_pointer_property (ecma_context_t *context_p, /**< JJS context */
                                     ecma_object_t *obj_p, /**< object to delete property from */
                                     const jjs_object_native_info_t *native_info_p) /**< native type info */
{
  if (ecma_op_object_is_fast_array (obj_p))
  {
  ...
  first_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_native_pointer_chain_t, value_p->value);

  /* There should be at least 2 native pointers in the chain */
  JJS_ASSERT (first_p != NULL && first_p->next_p != NULL);
  ...
  do
  {
    if (item_p->data.native_info_p == native_info_p)
    {
      if (prev_p == NULL)
      {
        ...
        ECMA_SET_INTERNAL_VALUE_POINTER (context_p, value_p->value, first_p);
      }
      else
      {
      ...
      *native_pointer_p = first_p->data;
      ...
      jmem_heap_free_block (context_p, first_p, sizeof (ecma_native_pointer_chain_t));
      return true;
    }
