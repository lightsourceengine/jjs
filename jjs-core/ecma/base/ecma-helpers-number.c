...
JJS_STATIC_ASSERT (sizeof (ecma_value_t) == sizeof (ecma_integer_value_t),
                     size_of_ecma_value_t_must_be_equal_to_the_size_of_ecma_integer_value_t);

JJS_STATIC_ASSERT (ECMA_DIRECT_SHIFT == ECMA_VALUE_SHIFT + 1, currently_directly_encoded_values_has_one_extra_flag);

JJS_STATIC_ASSERT (((1 << (ECMA_DIRECT_SHIFT - 1)) | ECMA_TYPE_DIRECT) == ECMA_DIRECT_TYPE_SIMPLE_VALUE,
                     currently_directly_encoded_values_start_after_direct_type_simple_value);

JJS_STATIC_ASSERT (sizeof (ecma_number_t) == sizeof (ecma_binary_num_t),
 ...
 */
extern inline ecma_binary_num_t JJS_ATTR_ALWAYS_INLINE JJS_ATTR_CONST
 ...
 * @return ecma-number
 */
extern inline ecma_number_t JJS_ATTR_ALWAYS_INLINE JJS_ATTR_CONST
 ...
 *         false - otherwise
 */
extern inline bool JJS_ATTR_ALWAYS_INLINE JJS_ATTR_CONST
 ...
 */
extern inline uint32_t JJS_ATTR_ALWAYS_INLINE JJS_ATTR_CONST
 ...
 */
extern inline uint64_t JJS_ATTR_ALWAYS_INLINE JJS_ATTR_CONST
...
{
  JJS_ASSERT ((biased_exp & ~((1u << ECMA_NUMBER_BIASED_EXP_WIDTH) - 1)) == 0);
  JJS_ASSERT ((fraction & ~((1ull << ECMA_NUMBER_FRACTION_WIDTH) - 1)) == 0);
...
ecma_number_is_nan (ecma_number_t num) /**< ecma-number */
{
  ...
  JJS_ASSERT (is_nan == is_nan_ieee754);
#endif /* !JJS_NDEBUG */
...
{
  JJS_ASSERT (!ecma_number_is_nan (num));
...
ecma_number_is_zero (ecma_number_t num) /**< ecma-number */
{
  ...
  JJS_ASSERT (is_zero == is_zero_ieee754);
#endif /* !JJS_NDEBUG */
...
{
  JJS_ASSERT (ecma_number_is_finite (left_num) && !ecma_number_is_zero (left_num));
  JJS_ASSERT (ecma_number_is_finite (right_num) && !ecma_number_is_zero (right_num));
                       ...
                       ecma_integer_value_t right_integer) /**< right operand */
{
  ...
  if (JJS_UNLIKELY ((left_integer & (left_integer - 1)) == 0))
  {
  ...
  if (JJS_UNLIKELY ((right_integer & (right_integer - 1)) == 0))
  {
