...
JJS_STATIC_ASSERT (ECMA_OBJECT_TYPE_MASK >= ECMA_OBJECT_TYPE__MAX - 1,
                     ecma_object_types_must_be_lower_than_the_container_mask);

JJS_STATIC_ASSERT (ECMA_OBJECT_TYPE_MASK >= ECMA_LEXICAL_ENVIRONMENT_TYPE__MAX,
                     ecma_lexical_environment_types_must_be_lower_than_the_container_mask);

JJS_STATIC_ASSERT (ECMA_OBJECT_FLAG_EXTENSIBLE == ECMA_OBJECT_TYPE_MASK + 1,
                     ecma_extensible_flag_must_follow_the_object_type);

JJS_STATIC_ASSERT (ECMA_OBJECT_REF_ONE == (ECMA_OBJECT_FLAG_EXTENSIBLE << 1),
                     ecma_object_ref_one_must_follow_the_extensible_flag);

JJS_STATIC_ASSERT ((ECMA_OBJECT_MAX_REF + ECMA_OBJECT_REF_ONE) == ECMA_OBJECT_REF_MASK,
                     ecma_object_max_ref_does_not_fill_the_remaining_bits);

JJS_STATIC_ASSERT ((ECMA_OBJECT_REF_MASK & (ECMA_OBJECT_TYPE_MASK | ECMA_OBJECT_FLAG_EXTENSIBLE)) == 0,
                     ecma_object_ref_mask_overlaps_with_object_type_or_extensible);

JJS_STATIC_ASSERT (ECMA_PROPERTY_FLAGS_MASK == ECMA_PROPERTY_CONFIGURABLE_ENUMERABLE_WRITABLE,
...
JJS_STATIC_ASSERT ((int) ECMA_OBJECT_TYPE_BUILT_IN_GENERAL == ((int) ECMA_OBJECT_TYPE_GENERAL | 0x1)
                       && ((int) ECMA_OBJECT_TYPE_GENERAL & 0x1) == 0,
                     ecma_object_type_built_in_general_has_unexpected_value);
JJS_STATIC_ASSERT ((int) ECMA_OBJECT_TYPE_BUILT_IN_CLASS == ((int) ECMA_OBJECT_TYPE_CLASS | 0x1)
                       && ((int) ECMA_OBJECT_TYPE_CLASS & 0x1) == 0,
                     ecma_object_type_built_in_class_has_unexpected_value);
JJS_STATIC_ASSERT ((int) ECMA_OBJECT_TYPE_BUILT_IN_ARRAY == ((int) ECMA_OBJECT_TYPE_ARRAY | 0x1)
 ...
 */
extern inline ecma_object_type_t JJS_ATTR_PURE
 ...
 */
extern inline ecma_object_base_type_t JJS_ATTR_PURE
 ...
 */
extern inline ecma_lexical_environment_type_t JJS_ATTR_PURE
                                 ...
                                 const ecma_object_t *object_p) /**< object-bound lexical environment */
{
  JJS_ASSERT (object_p != NULL);
  JJS_ASSERT (ecma_is_lexical_environment (object_p));
  JJS_ASSERT (ecma_get_lex_env_type (object_p) == ECMA_LEXICAL_ENVIRONMENT_THIS_OBJECT_BOUND
...
{
  JJS_ASSERT (ECMA_PROPERTY_PAIR_ITEM_COUNT == 2);
  ...
  if (*property_list_head_p != ECMA_NULL_POINTER)
  {
    ...
    JJS_ASSERT (ECMA_PROPERTY_IS_PROPERTY_PAIR (first_property_p));
    ...
    if (first_property_p->types[0] == ECMA_PROPERTY_TYPE_DELETED)
    {
      ...
      JJS_ASSERT (ECMA_PROPERTY_VALUE_PTR (property_p) == first_property_pair_p->values + 0);
  ...
  JJS_ASSERT (ECMA_PROPERTY_VALUE_PTR (property_p) == first_property_pair_p->values + 1);
                          ...
                          ecma_string_t *name_p) /**< property's name */
{
  ...
  if (ECMA_IS_DIRECT_STRING (name_p))
  {
    ...
    while (prop_iter_cp != JMEM_CP_NULL)
    {
      ...
      {
        JJS_ASSERT (ECMA_PROPERTY_IS_NAMED_PROPERTY (prop_iter_p->types[0]));
      ...
      {
        JJS_ASSERT (ECMA_PROPERTY_IS_NAMED_PROPERTY (prop_iter_p->types[1]));
                      ...
                      ecma_property_value_t *prop_value_p) /**< property value reference */
{
  ...
  while (cur_prop_cp != JMEM_CP_NULL)
  {
    ...
    for (uint32_t i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
      ...
      {
        JJS_ASSERT (ECMA_PROPERTY_IS_NAMED_PROPERTY (cur_prop_p->types[i]));
        ...
        JJS_ASSERT (cur_prop_p->types[i] == ECMA_PROPERTY_TYPE_DELETED);
                                          ...
                                          bool is_data) /**< property should be data property */
{
  ...
  while (prop_iter_cp != JMEM_CP_NULL)
  {
    ...
    for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
      if ((prop_pair_p->values + i) == prop_value_p)
      {
        JJS_ASSERT (is_data == ((prop_pair_p->header.types[i] & ECMA_PROPERTY_FLAG_DATA) != 0));
        return;
      }
    }
...
{
  JJS_ASSERT (property & ECMA_PROPERTY_FLAG_DATA);
...
{
  JJS_ASSERT (ECMA_PROPERTY_IS_RAW_DATA (*property_p));
...
{
  JJS_ASSERT (ECMA_PROPERTY_IS_NAMED_PROPERTY (property));
...
{
  JJS_ASSERT (ECMA_PROPERTY_IS_NAMED_PROPERTY (*property_p));
                               ...
                               ecma_property_descriptor_t *prop_desc_p) /**< property descriptor */
{
  ...
  if ((prop_desc_p->flags & JJS_PROP_IS_SET_DEFINED) && prop_desc_p->set_p != NULL)
  {
...
{
  if (JJS_LIKELY (primitve_p->refs_and_type < ECMA_EXTENDED_PRIMITIVE_MAX_REF))
  {
                      ...
                      ecma_extended_primitive_t *error_ref_p) /**< error reference */
{
  ...
  if (error_ref_p->refs_and_type < ECMA_EXTENDED_PRIMITIVE_REF_ONE)
  {
    ...
    jmem_heap_free_block (context_p, error_ref_p, sizeof (ecma_extended_primitive_t));
  }
                       ...
                       uint32_t options) /**< ECMA_ERROR_API_* options */
{
  ...
  error_ref_p = jmem_heap_alloc_block (context_p, sizeof (ecma_extended_primitive_t));
                   ...
                   ecma_value_t script_value) /**< script value */
{
  ...
  {
    JJS_ASSERT (!(type & CBC_SCRIPT_HAS_FUNCTION_ARGUMENTS));
    JJS_ASSERT (ecma_is_value_object (CBC_SCRIPT_GET_IMPORT_META (script_p, type)));
...
ecma_compiled_code_resolve_arguments_start (const ecma_compiled_code_t *bytecode_header_p) /**< compiled code */
{
  ...
  if (JJS_LIKELY (!(bytecode_header_p->status_flags & CBC_CODE_FLAGS_UINT16_ARGUMENTS)))
  {
