...
ecma_module_create (ecma_context_t *context_p) /**< JJS context */
{
  ...
  ext_object_p->u.cls.u1.module_state = JJS_MODULE_STATE_UNLINKED;
                           ...
                           ecma_string_t *name_p) /**< referenced value */
{
  ...
  switch (resolve_result_p->result_type)
  {
    ...
    default:
    {
      JJS_ASSERT (resolve_result_p->result_type == ECMA_MODULE_RESOLVE_NOT_FOUND
...
ecma_module_resolve_update (ecma_module_resolve_result_t *resolve_result_p, /**< [in,out] resolve result */
                            ecma_value_t result) /**< result value */
{
  JJS_ASSERT (resolve_result_p->result_type != ECMA_MODULE_RESOLVE_AMBIGUOUS
                && resolve_result_p->result_type != ECMA_MODULE_RESOLVE_ERROR);
  ...
  JJS_ASSERT (resolve_result_p->result_type == ECMA_MODULE_RESOLVE_FOUND);
                            ...
                            ecma_module_resolve_result_t *resolve_result_p) /**< [out] resolve result */
{
  ...
  do
  {
    ...
    if (current_module_p->header.u.cls.u1.module_state == JJS_MODULE_STATE_ERROR)
    {
      resolve_result_p->result_type = ECMA_MODULE_RESOLVE_ERROR;
      goto exit;
    }
                      ...
                      ecma_module_t *module_p) /**< module */
{
  ...
  JJS_ASSERT (module_p->header.u.cls.u1.module_state == JJS_MODULE_STATE_LINKED);
  JJS_ASSERT (module_p->scope_p != NULL);

  module_p->header.u.cls.u1.module_state = JJS_MODULE_STATE_EVALUATING;
...
//      scoped variable. however, ecma_op_has_binding() considers the global namespace and
//      results in a shadow error if the symbol shadows something in the global namespace.
//      this is not correct behavior. skipping the binding check gives the correct behavior.
//      need to do some research on the root cause
//
...
restart:
  ...
  while (true)
  {
    ...
    while (node_p != NULL)
    {
      ...
      if (module_p->header.u.cls.u1.module_state == JJS_MODULE_STATE_LINKING)
      {
...
error:
  JJS_ASSERT (last_p != NULL);

  do
  {
    ecma_module_stack_item_t *prev_p = last_p->prev_p;

    if (last_p->module_p->header.u.cls.u1.module_state != JJS_MODULE_STATE_ERROR)
    {
      JJS_ASSERT (last_p->module_p->header.u.cls.u1.module_state == JJS_MODULE_STATE_LINKING);
                            ...
                            ecma_module_t *module_p) /**< module */
{
  ...
  JJS_ASSERT (state != JJS_MODULE_STATE_INVALID);
