...
{
  JJS_ASSERT (!ecma_number_is_nan (time));
...
ecma_date_month_from_time (ecma_number_t time) /**< time value */
{
  ...
  JJS_ASSERT (day_within_year >= 0 && day_within_year < ECMA_DATE_DAYS_IN_LEAP_YEAR);
...
{
  JJS_ASSERT (context_p->platform.time_local_tza != NULL);
  int32_t tza;
  jjs_status_t status = context_p->platform.time_local_tza (time, &tza);
