                                           ...
                                           ecma_length_t *number_p) /**< [out] ecma_length_t */
{
  ...
  *number_p = ((to_int < 0) ? (ecma_length_t) JJS_MAX (((ecma_number_t) length + to_int), 0)
                            : (ecma_length_t) JJS_MIN (to_int, (ecma_number_t) length));
                                            ...
                                            uint32_t *number_p) /**< [out] uint32_t number */
{
  ...
  *number_p = ((to_int < 0) ? (uint32_t) JJS_MAX ((ecma_number_t) length + to_int, 0)
                            : (uint32_t) JJS_MIN (to_int, (ecma_number_t) length));
                                            ...
                                            uint32_t position) /**< start_position */
{
  ...
  if (original_str_size >= search_str_size)
  {
    ...
    for (uint32_t i = original_length; i > position; i--)
    {
  ...
  while (curr_p < replace_end_p)
  {
    if (*curr_p++ == LIT_CHAR_DOLLAR_SIGN)
    {
      ...
      switch (c)
      {
        ...
        {
          if (JJS_UNLIKELY (ctx_p->matched_p == NULL))
          {
            JJS_ASSERT (ctx_p->capture_count == 0);
            JJS_ASSERT (ctx_p->u.collection_p != NULL);
            JJS_ASSERT (ctx_p->u.collection_p->item_count > 0);
          ...
          if (lit_char_is_decimal_digit (c))
          {
            ...
            if (idx > 0 && idx < capture_count)
            {
              if (ctx_p->capture_count > 0)
              {
                ...
                JJS_ASSERT (ctx_p->u.captures_p != NULL);
