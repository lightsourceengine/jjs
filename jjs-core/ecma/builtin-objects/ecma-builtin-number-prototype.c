                                                ...
                                                uint32_t arguments_list_len) /**< number of arguments */
{
  ...
  if (fraction_part > 0.0)
  {
    ...
    precision = JJS_MAX (precision, ECMA_NUMBER_MIN_VALUE);
    ...
    do
    {
      ...
      JJS_ASSERT (fraction_cursor_p < fraction_digits + NUMBER_TO_STRING_MAX_DIGIT_COUNT);
  ...
  if (fraction_cursor_p != fraction_digits)
  {
    ...
    JJS_ASSERT (fraction_digit_count > 0);
...
ecma_builtin_number_prototype_object_value_of (ecma_value_t this_arg) /**< this argument */
{
  ...
  else if (ecma_is_value_object (this_arg))
  {
    ...
    if (ecma_object_class_is (object_p, ECMA_OBJECT_CLASS_NUMBER))
    {
      ...
      JJS_ASSERT (ecma_is_value_number (ext_object_p->u.cls.u3.value));
                                                        ...
                                                        number_routine_mode_t mode) /**< number routine mode */
{
  ...
  else
  {
    ...
    copied_digits = JJS_MIN (JJS_MIN (num_of_digits, result_digits), (lit_utf8_size_t) exponent);
  ...
  if (result_digits > 0)
  {
    ...
    lit_utf8_size_t to_copy = JJS_MIN (num_of_digits, result_digits);
