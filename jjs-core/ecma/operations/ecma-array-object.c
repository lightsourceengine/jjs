...
ecma_op_alloc_array_object (ecma_context_t *context_p, /**< JJS context */
                            uint32_t length) /**< length of the new array */
{
  ...
  ecma_object_t *object_p =
    ecma_create_object (context_p, array_prototype_object_p, sizeof (ecma_extended_object_t), ECMA_OBJECT_TYPE_ARRAY);
...
{
  JJS_ASSERT (ecma_get_object_base_type ((ecma_object_t *) array_p) == ECMA_OBJECT_BASE_TYPE_ARRAY);
 ...
 */
ecma_object_t *
ecma_op_new_array_object (ecma_context_t *context_p, /**< JJS context */
                          uint32_t length) /**< length of the new array */
{
  ecma_object_t *object_p = ecma_op_alloc_array_object (context_p, length);
  ...
  if (length > 0)
  {
    ...
    values_p = (ecma_value_t *) jmem_heap_alloc_block_null_on_error (context_p, aligned_length * sizeof (ecma_value_t));

    if (JJS_UNLIKELY (values_p == NULL))
    {
      return object_p;
    }
  }
  ...
  JJS_ASSERT (object_p->u1.property_list_cp == JMEM_CP_NULL);
  ECMA_SET_POINTER (context_p, object_p->u1.property_list_cp, values_p);
  return object_p;
 ...
 */
ecma_object_t *
ecma_op_new_array_object_from_length (ecma_context_t *context_p, /**< JJS context */
                                      ecma_length_t length) /**< length of the new array */
{
  if (length > UINT32_MAX)
  {
    ecma_raise_range_error (context_p, ECMA_ERR_INVALID_ARRAY_LENGTH);
    return NULL;
  }

  return ecma_op_new_array_object (context_p, (uint32_t) length);
 ...
 */
ecma_value_t
ecma_op_new_array_object_from_buffer (ecma_context_t *context_p, /**< JJS context */
                                      const ecma_value_t *args_p, /**< array element list */
                                      uint32_t length) /**< number of array elements */
{
  ...
  if (values_p != NULL)
  {
    ...
    {
      JJS_ASSERT (!ecma_is_value_array_hole (args_p[i]));
      values_p[i] = ecma_copy_value_if_not_object (context_p, args_p[i]);
    }
  ...
  else
  {
    for (uint32_t i = 0; i < length; i++)
    {
      ...
      prop_value_p->value = ecma_copy_value_if_not_object (context_p, args_p[i]);
    }
  }
 ...
 */
ecma_value_t
ecma_op_new_array_object_from_collection (ecma_context_t *context_p, /**< JJS context */
                                          ecma_collection_t *collection_p, /**< collection to create array from */
                                                                   ...
                                                                   false - otherwise */
{
  ...
  buffer_p = jmem_heap_realloc_block (context_p, buffer_p, old_size, aligned_length * sizeof (ecma_value_t));
  object_p = ecma_op_alloc_array_object (context_p, item_count);

  ecma_extended_object_t *ext_obj_p = (ecma_extended_object_t *) object_p;
  ext_obj_p->u.array.length_prop_and_hole_count |= ECMA_FAST_ARRAY_FLAG;
  JJS_ASSERT (object_p->u1.property_list_cp == JMEM_CP_NULL);
  JJS_ASSERT (ext_obj_p->u.array.length_prop_and_hole_count < ECMA_FAST_ARRAY_HOLE_ONE);
  ECMA_SET_POINTER (context_p, object_p->u1.property_list_cp, buffer_p);

  if (JJS_UNLIKELY (unref_objects))
  {
    for (uint32_t i = 0; i < item_count; i++)
    {
      ...
      ecma_deref_if_object (context_p, buffer_p[i]);
    }
  }
 ...
 */
void
ecma_fast_array_convert_to_normal (ecma_context_t *context_p, /**< JJS context */
                                   ecma_object_t *object_p) /**< fast access mode array object */
{
  ...
  while (index >= 0)
  {
    ...
    if (prop_index == 1)
    {
      property_pair_p = ecma_alloc_property_pair (context_p);
      ...
      ECMA_SET_NON_NULL_POINTER (context_p, next_property_pair_cp, property_pair_p);
    }

    JJS_ASSERT (index <= ECMA_DIRECT_STRING_MAX_IMM);
  ...
  ECMA_SET_POINTER (context_p, object_p->u1.property_list_cp, property_pair_p);
 ...
 */
bool
ecma_fast_array_set_property (ecma_context_t *context_p, /**< JJS context */
                              ecma_object_t *object_p, /**< fast access mode array object */
                              ...
                              ecma_value_t value) /**< value to be set */
{
  ...
  if (JJS_LIKELY (index < old_length))
  {
    JJS_ASSERT (object_p->u1.property_list_cp != JMEM_CP_NULL);

    values_p = ECMA_GET_NON_NULL_POINTER (context_p, ecma_value_t, object_p->u1.property_list_cp);
    ...
    {
      ecma_free_value_if_not_object (context_p, values_p[index]);
    }

    values_p[index] = ecma_copy_value_if_not_object (context_p, value);
  ...
  if (JJS_UNLIKELY (new_holes > (ECMA_FAST_ARRAY_MAX_HOLE_COUNT - (old_holes >> ECMA_FAST_ARRAY_HOLE_SHIFT))))
  {
...
{
  JJS_ASSERT (ecma_op_object_is_fast_array (obj_p));
 ...
 */
ecma_value_t *
ecma_fast_array_extend (ecma_context_t *context_p, /**< JJS context */
                        ecma_object_t *object_p, /**< fast access mode array object */
                        uint32_t new_length) /**< new length of the fast access mode array */
{
  ...
  JJS_ASSERT (old_length < new_length);
  ...
  if (object_p->u1.property_list_cp == JMEM_CP_NULL)
  {
    new_values_p = jmem_heap_alloc_block (context_p, new_length_aligned * sizeof (ecma_value_t));
  }
  else
  {
    ecma_value_t *values_p = ECMA_GET_NON_NULL_POINTER (context_p, ecma_value_t, object_p->u1.property_list_cp);
    new_values_p = (ecma_value_t *) jmem_heap_realloc_block (context_p, values_p,
  ...
  ECMA_SET_NON_NULL_POINTER (context_p, object_p->u1.property_list_cp, new_values_p);
 ...
 */
bool
ecma_array_object_delete_property (ecma_context_t *context_p, /**< JJS context */
                                   ecma_object_t *object_p, /**< object */
                                   ecma_string_t *property_name_p) /**< property name */
{
  JJS_ASSERT (ecma_get_object_base_type (object_p) == ECMA_OBJECT_BASE_TYPE_ARRAY);
  ecma_extended_object_t *ext_obj_p = (ecma_extended_object_t *) object_p;
  ...
  JJS_ASSERT (index < ext_obj_p->u.array.length);
 ...
 */
uint32_t
ecma_delete_fast_array_properties (ecma_context_t *context_p, /**< JJS context */
                                   ecma_object_t *object_p, /**< fast access mode array */
                                   uint32_t new_length) /**< new length of the fast access mode array */
{
  JJS_ASSERT (ecma_op_object_is_fast_array (object_p));
  ...
  JJS_ASSERT (new_length < old_length);
  ...
  if (new_length == 0)
  {
    jmem_heap_free_block (context_p, values_p, old_aligned_length * sizeof (ecma_value_t));
  ...
  else
  {
    ...
    ecma_value_t *new_values_p;
    new_values_p = (ecma_value_t *) jmem_heap_realloc_block (context_p,
                                                             values_p,
    ...
    ECMA_SET_NON_NULL_POINTER (context_p, new_property_list_cp, new_values_p);
  }
 ...
 */
static void
ecma_fast_array_set_length (ecma_context_t *context_p, /**< JJS context */
                            ecma_object_t *object_p, /**< fast access mode array object */
                            uint32_t new_length) /**< new length of the fast access mode array object*/
{
  ...
  {
    ecma_fast_array_extend (context_p, object_p, new_length);
  }
 ...
 */
ecma_collection_t *
ecma_fast_array_object_own_property_keys (ecma_context_t *context_p, /**< JJS context */
                                          ecma_object_t *object_p, /**< fast access mode array object */
                                          jjs_property_filter_t filter) /**< property name filter options */
{
  JJS_ASSERT (ecma_op_object_is_fast_array (object_p));

  ecma_collection_t *ret_p = ecma_new_collection (context_p);
  ...
  if (!(filter & JJS_PROPERTY_FILTER_EXCLUDE_INTEGER_INDICES))
  {
    ...
    if (length != 0)
    {
      ...
      for (uint32_t i = 0; i < length; i++)
      {
        ...
        ecma_string_t *index_str_p = ecma_new_ecma_string_from_uint32 (context_p, i);

        ecma_collection_push_back (context_p, ret_p, ecma_make_string_value (context_p, index_str_p));
      }
    }
  }
  ...
  {
    ecma_collection_push_back (context_p, ret_p, ecma_make_magic_string_value (LIT_MAGIC_STRING_LENGTH));
  }
 ...
 */
ecma_object_t *
ecma_op_array_species_create (ecma_context_t *context_p, /**< JJS context */
                              ecma_object_t *original_array_p, /**< The object from whom the new array object
                                                                *   is being created */
                              ecma_length_t length) /**< length of the array */
{
  ecma_value_t constructor = ECMA_VALUE_UNDEFINED;
  ecma_value_t original_array = ecma_make_object_value (context_p, original_array_p);

  ecma_value_t is_array = ecma_is_value_array (context_p, original_array);
  ...
  {
    constructor = ecma_op_object_get_by_magic_id (context_p, original_array_p, LIT_MAGIC_STRING_CONSTRUCTOR);
    ...
    if (ecma_is_value_object (constructor))
    {
      ...
      constructor = ecma_op_object_get_by_symbol_id (context_p, ctor_object_p, LIT_GLOBAL_SYMBOL_SPECIES);
  ...
  {
    return ecma_op_new_array_object_from_length (context_p, length);
  }

  if (!ecma_is_constructor (context_p, constructor))
  {
    ecma_free_value (context_p, constructor);
    ecma_raise_type_error (context_p, ECMA_ERR_INVALID_SPECIES_CONSTRUCTOR);
    return NULL;
  }

  ecma_value_t len_val = ecma_make_length_value (context_p, length);
  ecma_object_t *ctor_object_p = ecma_get_object_from_value (context_p, constructor);
  ecma_value_t ret_val = ecma_op_function_construct (context_p, ctor_object_p, ctor_object_p, &len_val, 1);
  ...
  return ecma_get_object_from_value (context_p, ret_val);
 ...
 */
ecma_value_t
ecma_op_create_array_iterator (ecma_context_t *context_p, /**< JJS context */
 ...
 */
static uint32_t
ecma_delete_array_properties (ecma_context_t *context_p, /**< JJS context */
                              ecma_object_t *object_p, /**< object */
                              ...
                              uint32_t old_length) /**< old length */
{
  ...
  if (ecma_op_object_is_fast_array (object_p))
  {
    return ecma_delete_fast_array_properties (context_p, object_p, new_length);
  }
  ...
  current_prop_p = ECMA_GET_NON_NULL_POINTER (context_p, ecma_property_header_t, current_prop_cp);
  ...
  {
    current_prop_p = ECMA_GET_NON_NULL_POINTER (context_p, ecma_property_header_t, current_prop_cp);
    JJS_ASSERT (ECMA_PROPERTY_IS_PROPERTY_PAIR (current_prop_p));
    ...
    for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
      ...
      {
        uint32_t index = ecma_string_get_property_index (context_p, current_prop_p->types[i], prop_pair_p->names_cp[i]);
...
#if JJS_PROPERTY_HASHMAP
  JJS_ASSERT (current_prop_cp != JMEM_CP_NULL);
  ...
  while (current_prop_cp != JMEM_CP_NULL)
  {
    ...
    for (uint32_t i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
          ...
          && ecma_is_property_configurable (current_prop_p->types[i]))
      {
        ...
        if (index < old_length && index >= new_length)
        {
          ...
          {
            hashmap_status =
              ecma_property_hashmap_delete (context_p, object_p, prop_pair_p->names_cp[i], current_prop_p->types + i);
          }
#endif /* JJS_PROPERTY_HASHMAP */
        ...
        && current_prop_p->types[1] == ECMA_PROPERTY_TYPE_DELETED)
    {
      ...
      ecma_dealloc_property_pair (context_p, (ecma_property_pair_t *) current_prop_p);
 ...
 */
ecma_value_t
ecma_op_array_object_set_length (ecma_context_t *context_p, /**< JJS context */
                                 ecma_object_t *object_p, /**< the array object */
                                 ecma_value_t new_value, /**< new length value */
                                 uint16_t flags) /**< property descriptor flags */
{
  ecma_number_t new_len_num;
  ecma_value_t completion = ecma_op_to_number (context_p, new_value, &new_len_num);
  ...
  JJS_ASSERT (!ECMA_IS_VALUE_ERROR (completion));

  if (ecma_is_value_object (new_value))
  {
    ecma_value_t compared_num_val = ecma_op_to_number (context_p, new_value, &new_len_num);
  ...
  if (flags
      & (JJS_PROP_IS_CONFIGURABLE | JJS_PROP_IS_ENUMERABLE | JJS_PROP_IS_GET_DEFINED | JJS_PROP_IS_SET_DEFINED))
  {
    return ecma_raise_property_redefinition (context_p, ecma_get_magic_string (LIT_MAGIC_STRING_LENGTH), flags);
  }
  ...
  if (new_len_num == old_len_uint32)
  {
    ...
    if (flags & JJS_PROP_IS_WRITABLE_DEFINED)
    {
      if (!(flags & JJS_PROP_IS_WRITABLE))
      {
  ...
  {
    current_len_uint32 = ecma_delete_array_properties (context_p, object_p, new_len_uint32, old_len_uint32);
  }
  else if (ecma_op_object_is_fast_array (object_p))
  {
    ecma_fast_array_set_length (context_p, object_p, new_len_uint32);
  }

  ext_object_p->u.array.length = current_len_uint32;

  if ((flags & JJS_PROP_IS_WRITABLE_DEFINED) && !(flags & JJS_PROP_IS_WRITABLE))
  {
    if (ecma_op_array_is_fast_array (ext_object_p))
    {
      ecma_fast_array_convert_to_normal (context_p, object_p);
...
#define ECMA_FAST_ARRAY_DATA_PROP_FLAGS                                                               \
  (JJS_PROP_IS_VALUE_DEFINED | JJS_PROP_IS_ENUMERABLE_DEFINED | JJS_PROP_IS_ENUMERABLE          \
   | JJS_PROP_IS_CONFIGURABLE_DEFINED | JJS_PROP_IS_CONFIGURABLE | JJS_PROP_IS_WRITABLE_DEFINED \
 ...
 */
ecma_value_t
ecma_op_array_object_define_own_property (ecma_context_t *context_p, /**< JJS context */
                                          ecma_object_t *object_p, /**< the array object */
                                          ecma_string_t *property_name_p, /**< property name */
                                          const ecma_property_descriptor_t *property_desc_p) /**< property descriptor */
{
  ...
  {
    JJS_ASSERT ((property_desc_p->flags & JJS_PROP_IS_CONFIGURABLE_DEFINED)
                  || !(property_desc_p->flags & JJS_PROP_IS_CONFIGURABLE));
    JJS_ASSERT ((property_desc_p->flags & JJS_PROP_IS_ENUMERABLE_DEFINED)
                  || !(property_desc_p->flags & JJS_PROP_IS_ENUMERABLE));
    JJS_ASSERT ((property_desc_p->flags & JJS_PROP_IS_WRITABLE_DEFINED)
                  || !(property_desc_p->flags & JJS_PROP_IS_WRITABLE));

    if (property_desc_p->flags & JJS_PROP_IS_VALUE_DEFINED)
    {
      return ecma_op_array_object_set_length (context_p, object_p, property_desc_p->value, property_desc_p->flags);
    }

    ecma_extended_object_t *ext_object_p = (ecma_extended_object_t *) object_p;
    ecma_value_t length_value = ecma_make_uint32_value (context_p, ext_object_p->u.array.length);

    ecma_value_t result = ecma_op_array_object_set_length (context_p, object_p, length_value, property_desc_p->flags);

    ecma_fast_free_value (context_p, length_value);
    return result;
  }
  ...
  if (ecma_op_object_is_fast_array (object_p))
  {
    if ((property_desc_p->flags & ECMA_FAST_ARRAY_DATA_PROP_FLAGS) == ECMA_FAST_ARRAY_DATA_PROP_FLAGS)
    {
      ...
      else if (ecma_fast_array_set_property (context_p, object_p, index, property_desc_p->value))
      {
        return ECMA_VALUE_TRUE;
      }

      JJS_ASSERT (!ecma_op_array_is_fast_array (ext_object_p));
    }
    else
    {
      ecma_fast_array_convert_to_normal (context_p, object_p);
  ...
  {
    return ecma_op_general_object_define_own_property (context_p, object_p, property_name_p, property_desc_p);
  }
  ...
  prop_desc.flags &= (uint16_t) ~JJS_PROP_SHOULD_THROW;

  ecma_value_t completion = ecma_op_general_object_define_own_property (context_p, object_p, property_name_p, &prop_desc);
...
{
  JJS_ASSERT (ecma_get_object_base_type (array_p) == ECMA_OBJECT_BASE_TYPE_ARRAY);
 ...
 */
ecma_value_t
ecma_array_object_to_string (ecma_context_t *context_p, /**< JJS context */
                             ecma_value_t this_arg) /**< this argument */
{
  ...
  /* 2. */
  ecma_value_t join_value = ecma_op_object_get_by_magic_id (context_p, obj_p, LIT_MAGIC_STRING_JOIN);
  ...
  {
    /* 4. */
    ecma_object_t *join_func_obj_p = ecma_get_object_from_value (context_p, join_value);

    ret_value = ecma_op_function_call (context_p, join_func_obj_p, this_arg, NULL, 0);
  }

  ecma_free_value (context_p, join_value);
