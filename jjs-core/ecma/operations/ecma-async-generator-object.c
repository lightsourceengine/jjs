 ...
 */
ecma_value_t
ecma_async_generator_enqueue (ecma_context_t *context_p, /**< JJS context */
...
{
  ecma_async_generator_task_t *task_p = jmem_heap_alloc_block (context_p, sizeof (ecma_async_generator_task_t));

  ECMA_SET_INTERNAL_VALUE_ANY_POINTER (context_p, task_p->next, NULL);
  task_p->operation_value = ecma_copy_value_if_not_object (context_p, value);
  task_p->operation_type = (uint8_t) operation;

  ecma_value_t result = ecma_op_create_promise_object (context_p, ECMA_VALUE_EMPTY, ECMA_VALUE_UNDEFINED, NULL);
  ...
  {
    ECMA_SET_INTERNAL_VALUE_POINTER (context_p, async_generator_object_p->extended_object.u.cls.u3.head, task_p);
    ...
    ecma_async_generator_run (context_p, async_generator_object_p);
    return result;
  }

  /* Append the new task at the end. */
  ecma_async_generator_task_t *prev_task_p;
  prev_task_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_async_generator_task_t, head);

  while (!ECMA_IS_INTERNAL_VALUE_NULL (prev_task_p->next))
  {
    prev_task_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_async_generator_task_t, prev_task_p->next);
  }

  ECMA_SET_INTERNAL_VALUE_POINTER (context_p, prev_task_p->next, task_p);
  return result;
 ...
 */
static ecma_value_t
ecma_async_yield_call (ecma_context_t *context_p, /**< JJS context */
                       ecma_value_t function, /**< function (takes reference) */
                       vm_executable_object_t *async_generator_object_p, /**< async generator */
                       ecma_value_t argument) /**< argument passed to the function */
{
  ...
  {
    result = ecma_op_function_validated_call (context_p, function, iterator, NULL, 0);
  }
  else
  {
    result = ecma_op_function_validated_call (context_p, function, iterator, &argument, 1);
  }

  ecma_free_value (context_p, function);
  ...
  return ecma_promise_async_await (context_p, (ecma_extended_object_t *) async_generator_object_p, result);
 ...
 */
static ecma_value_t
ecma_async_yield_throw (ecma_context_t *context_p, /**< JJS context */
                        vm_executable_object_t *async_generator_object_p, /**< async generator */
                        ecma_value_t value) /**< thrown value */
{
  ecma_object_t *obj_p = ecma_get_object_from_value (context_p, async_generator_object_p->iterator);
  ecma_value_t result = ecma_op_object_get_by_magic_id (context_p, obj_p, LIT_MAGIC_STRING_THROW);
  ...
  {
    result = ecma_op_object_get_by_magic_id (context_p, obj_p, LIT_MAGIC_STRING_RETURN);
    ...
    result = ecma_async_yield_call (context_p, result, async_generator_object_p, ECMA_VALUE_EMPTY);
  ...
  result = ecma_async_yield_call (context_p, result, async_generator_object_p, value);
 ...
 */
ecma_value_t
ecma_async_generator_run (ecma_context_t *context_p, /**< JJS context */
                          vm_executable_object_t *async_generator_object_p) /**< async generator */
{
  ...
  ecma_value_t head = async_generator_object_p->extended_object.u.cls.u3.head;
  ecma_async_generator_task_t *task_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_async_generator_task_t, head);
  ecma_value_t result;
      ...
      & ECMA_EXECUTABLE_OBJECT_DO_AWAIT_OR_YIELD)
  {
    switch (task_p->operation_type)
    {
      ...
      {
        result = ecma_op_iterator_next (context_p,
                                        async_generator_object_p->iterator,
        ...
        result = ecma_promise_async_await (context_p, (ecma_extended_object_t *) async_generator_object_p, result);
      ...
      {
        result = ecma_async_yield_throw (context_p, async_generator_object_p, task_p->operation_value);
        break;
      }
      default:
      {
        JJS_ASSERT (task_p->operation_type == ECMA_ASYNC_GENERATOR_DO_RETURN);

        result = ecma_copy_value (context_p, task_p->operation_value);
        result = ecma_promise_async_await (context_p, (ecma_extended_object_t *) async_generator_object_p, result);
    ...
    JJS_ASSERT (async_generator_object_p->frame_ctx.stack_top_p[-1] == ECMA_VALUE_UNDEFINED
  ...
  else
  {
    ...
    ecma_ref_if_object (context_p, result);
    task_p->operation_value = ECMA_VALUE_UNDEFINED;
  }
  ...
  {
    JJS_ASSERT (head == async_generator_object_p->extended_object.u.cls.u3.head);
    ecma_async_generator_finalize (context_p, async_generator_object_p, result);
 ...
 */
void
ecma_async_generator_finalize (ecma_context_t *context_p, /**< JJS context */
                               vm_executable_object_t *async_generator_object_p, /**< async generator */
                               ecma_value_t value) /**< final value (takes reference) */
{
  ecma_value_t next = async_generator_object_p->extended_object.u.cls.u3.head;
  ecma_async_generator_task_t *task_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_async_generator_task_t, next);

  if (ECMA_IS_VALUE_ERROR (value))
  {
    value = jcontext_take_exception (context_p);
    ecma_reject_promise (context_p, task_p->promise, value);
  }
  else
  {
    ecma_value_t result = ecma_create_iter_result_object (context_p, value, ECMA_VALUE_TRUE);
    ecma_fulfill_promise (context_p, task_p->promise, result);
  ...
  {
    task_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_async_generator_task_t, next);

    if (task_p->operation_type != ECMA_ASYNC_GENERATOR_DO_THROW)
    {
      value = ecma_create_iter_result_object (context_p, ECMA_VALUE_UNDEFINED, ECMA_VALUE_TRUE);
      ecma_fulfill_promise (context_p, task_p->promise, value);
      ecma_free_value (context_p, value);
    }
    else
    {
      ecma_reject_promise (context_p, task_p->promise, task_p->operation_value);
    }
 ...
 */
ecma_value_t
ecma_await_continue (ecma_context_t *context_p, /**< JJS context */
                     vm_executable_object_t *executable_object_p, /**< executable object */
                     ecma_value_t value) /**< job value (takes reference) */
{
  ...
  switch (state)
  {
    ...
    case ECMA_AWAIT_YIELD_NEXT_RETURN:
    {
      ...
      {
        ecma_free_value (context_p, value);
        return ecma_raise_type_error (context_p, ECMA_ERR_VALUE_RECEIVED_BY_YIELD_IS_NOT_OBJECT);
      }

      ecma_object_t *result_obj_p = ecma_get_object_from_value (context_p, value);
      ecma_value_t result = ecma_op_object_get_by_magic_id (context_p, result_obj_p, LIT_MAGIC_STRING_DONE);
      ...
      bool done = ecma_op_to_boolean (context_p, result);
      ecma_free_value (context_p, result);
      result = ecma_op_object_get_by_magic_id (context_p, result_obj_p, LIT_MAGIC_STRING_VALUE);
      ecma_deref_object (result_obj_p);
      ...
      if (!done)
      {
        ...
        return ecma_promise_async_await (context_p, (ecma_extended_object_t *) executable_object_p, result);
      }
    ...
    {
      ecma_object_t *obj_p = ecma_get_object_from_value (context_p, executable_object_p->iterator);
      ecma_value_t result = ecma_op_object_get_by_magic_id (context_p, obj_p, LIT_MAGIC_STRING_RETURN);
      ...
      result = ecma_async_yield_call (context_p, result, executable_object_p, value);
      ecma_free_value (context_p, value);
      ...
      JJS_ASSERT (result == ECMA_VALUE_UNDEFINED);
    ...
    case ECMA_AWAIT_YIELD_NEXT_VALUE:
    {
      ...
      opfunc_async_generator_yield (context_p, (ecma_extended_object_t *) executable_object_p, value);
      return ECMA_VALUE_UNDEFINED;
    }
    case ECMA_AWAIT_YIELD_OPERATION:
    {
      /* Currently this is always a throw exception case. */
      ecma_value_t result = ecma_async_yield_throw (context_p, executable_object_p, value);
      ecma_free_value (context_p, value);
      return result;
    }
    case ECMA_AWAIT_YIELD_CLOSE:
    {
      ...
      ecma_free_value (context_p, value);
      return ecma_raise_type_error (context_p, msg);
    }
    case ECMA_AWAIT_FOR_CLOSE:
    {
      ...
      {
        return ecma_raise_type_error (context_p, ECMA_ERR_ITERATOR_RETURN_RESULT_IS_NOT_OBJECT);
      }
      return ECMA_VALUE_EMPTY;
    }
    default:
    {
      JJS_ASSERT (state == ECMA_AWAIT_FOR_NEXT);
      JJS_ASSERT (VM_GET_CONTEXT_TYPE (executable_object_p->frame_ctx.stack_top_p[-1]) == VM_CONTEXT_FOR_AWAIT_OF);
      JJS_ASSERT (!(executable_object_p->frame_ctx.stack_top_p[-1] & VM_CONTEXT_CLOSE_ITERATOR));

      if (!ecma_is_value_object (value))
      {
        ecma_free_value (context_p, value);
        return ecma_raise_type_error (context_p, ECMA_ERR_VALUE_RECEIVED_BY_FOR_ASYNC_OF_IS_NOT_OBJECT);
      }
      ...
      JJS_ASSERT (ecma_is_value_object (stack_top_p[-3]));
      JJS_ASSERT (stack_top_p[-4] == ECMA_VALUE_UNDEFINED || ecma_is_value_object (stack_top_p[-4]));
      ...
      if (!done)
      {
        ...
        ecma_deref_if_object (context_p, result);
