...
ecma_bigint_number_to_bigint (ecma_number_t number) /**< ecma number */
{
  ...
  JJS_ASSERT (ECMA_BIGINT_NUMBER_TO_DIGITS_GET_DIGITS (result) == 0
...
ecma_bigint_to_number (ecma_value_t value) /**< BigInt value */
{
  ...
  if (digits_p[-1] == 1)
  {
    JJS_ASSERT (size > sizeof (ecma_bigint_digit_t));
...
{
  JJS_ASSERT (ecma_is_value_bigint (left_value));
  JJS_ASSERT (!ecma_number_is_nan (right_value));
  ...
  if (digits_size == 0)
  {
    JJS_ASSERT (result & ECMA_BIGINT_NUMBER_TO_DIGITS_HAS_FRACTION);
