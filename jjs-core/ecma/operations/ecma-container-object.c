...
ecma_op_create_internal_buffer (ecma_context_t *context_p)
{
  ecma_collection_t *collection_p = ecma_new_collection (context_p);
  ecma_collection_push_back (context_p, collection_p, (ecma_value_t) 0);
 ...
 */
static void
ecma_op_internal_buffer_append (ecma_context_t *context_p, /**< JJS context */
                                ecma_collection_t *container_p, /**< internal container pointer */
                                ecma_value_t key_arg, /**< key argument */
                                ecma_value_t value_arg, /**< value argument */
                                lit_magic_string_id_t lit_id) /**< class id */
{
  JJS_ASSERT (container_p != NULL);

  if (lit_id == LIT_MAGIC_STRING_WEAKMAP_UL || lit_id == LIT_MAGIC_STRING_MAP_UL)
  {
    ecma_value_t values[] = { ecma_copy_value_if_not_object (context_p, key_arg), ecma_copy_value_if_not_object (context_p, value_arg) };
    ecma_collection_append (context_p, container_p, values, 2);
  }
  else
  {
    ecma_collection_push_back (context_p, container_p, ecma_copy_value_if_not_object (context_p, key_arg));
  }
 ...
 */
static inline void
ecma_op_internal_buffer_update (ecma_context_t *context_p, /**< JJS context */
                                ecma_value_t *entry_p, /**< entry pointer */
                                ecma_value_t value_arg, /**< value argument */
                                lit_magic_string_id_t lit_id) /**< class id */
{
  JJS_ASSERT (entry_p != NULL);

  if (lit_id == LIT_MAGIC_STRING_WEAKMAP_UL || lit_id == LIT_MAGIC_STRING_MAP_UL)
  {
    ecma_free_value_if_not_object (context_p, ((ecma_container_pair_t *) entry_p)->value);

    ((ecma_container_pair_t *) entry_p)->value = ecma_copy_value_if_not_object (context_p, value_arg);
  }
 ...
 */
static void
ecma_op_internal_buffer_delete (ecma_context_t *context_p, /**< JJS context */
                                ecma_collection_t *container_p, /**< internal container pointer */
                                ...
                                lit_magic_string_id_t lit_id) /**< class id */
{
  ...
  ecma_free_value_if_not_object (context_p, entry_p->key);
  entry_p->key = ECMA_VALUE_EMPTY;

  if (lit_id == LIT_MAGIC_STRING_WEAKMAP_UL || lit_id == LIT_MAGIC_STRING_MAP_UL)
  {
    ecma_free_value_if_not_object (context_p, entry_p->value);
 ...
 */
static ecma_value_t *
ecma_op_internal_buffer_find (ecma_context_t *context_p, /**< JJS context */
                              ecma_collection_t *container_p, /**< internal container pointer */
                              ecma_value_t key_arg, /**< key argument */
                              lit_magic_string_id_t lit_id) /**< class id */
{
  ...
  for (uint32_t i = 0; i < entry_count; i += entry_size)
  {
    ...
    if (ecma_op_same_value_zero (context_p, *entry_p, key_arg, false))
    {
      return entry_p;
    }
  }
 ...
 */
static void
ecma_op_container_free_weakset_entries (ecma_context_t *context_p, /**< JJS context */
                                        ecma_object_t *object_p, /**< object pointer */
                                        ecma_collection_t *container_p) /** internal buffer pointer */
{
  ...
  for (uint32_t i = 0; i < entry_count; i += ECMA_CONTAINER_VALUE_SIZE)
  {
    ...
    ecma_op_object_unref_weak (context_p, ecma_get_object_from_value (context_p, *entry_p), ecma_make_object_value (context_p, object_p));
    ecma_op_container_remove_weak_entry (context_p, object_p, *entry_p);
 ...
 */
static void
ecma_op_container_free_weakmap_entries (ecma_context_t *context_p, /**< JJS context */
                                        ecma_object_t *object_p, /**< object pointer */
                                        ecma_collection_t *container_p) /**< internal buffer pointer */
{
  JJS_ASSERT (object_p != NULL);
  ...
  for (uint32_t i = 0; i < entry_count; i += ECMA_CONTAINER_PAIR_SIZE)
  {
    ...
    ecma_op_object_unref_weak (context_p, ecma_get_object_from_value (context_p, entry_p->key), ecma_make_object_value (context_p, object_p));
    ecma_op_container_remove_weak_entry (context_p, object_p, entry_p->key);
 ...
 */
static void
ecma_op_container_free_set_entries (ecma_context_t *context_p, ecma_collection_t *container_p)
{
  JJS_ASSERT (container_p != NULL);
  ...
  for (uint32_t i = 0; i < entry_count; i += ECMA_CONTAINER_VALUE_SIZE)
  {
    ...
    ecma_free_value_if_not_object (context_p, *entry_p);
    *entry_p = ECMA_VALUE_EMPTY;
  }
 ...
 */
static void
ecma_op_container_free_map_entries (ecma_context_t *context_p, ecma_collection_t *container_p)
{
  JJS_ASSERT (container_p != NULL);
 ...
 */
void
ecma_op_container_free_entries (ecma_context_t *context_p, ecma_object_t *object_p) /**< collection object pointer */
{
  JJS_ASSERT (object_p != NULL);
  ...
  switch (map_object_p->u.cls.u2.container_id)
  {
    ...
    {
      ecma_op_container_free_weakset_entries (context_p, object_p, container_p);
      break;
    }
    case LIT_MAGIC_STRING_WEAKMAP_UL:
    {
      ecma_op_container_free_weakmap_entries (context_p, object_p, container_p);
      break;
    }
    case LIT_MAGIC_STRING_SET_UL:
    {
      ecma_op_container_free_set_entries (context_p, container_p);
      break;
    }
    case LIT_MAGIC_STRING_MAP_UL:
    {
      ecma_op_container_free_map_entries (context_p, container_p);
      break;
    }
    default:
    {
 ...
 */
ecma_value_t
ecma_op_container_create (ecma_context_t *context_p, /**< JJS context */
                          ...
                          ecma_builtin_id_t proto_id) /**< prototype builtin id */
{
  ...
  JJS_ASSERT (lit_id == LIT_MAGIC_STRING_MAP_UL || lit_id == LIT_MAGIC_STRING_SET_UL
                || lit_id == LIT_MAGIC_STRING_WEAKMAP_UL || lit_id == LIT_MAGIC_STRING_WEAKSET_UL);
  JJS_ASSERT (context_p->current_new_target_p != NULL);

  ecma_object_t *proto_p = ecma_op_get_prototype_from_constructor (context_p, context_p->current_new_target_p, proto_id);
  ...
  ecma_collection_t *container_p = ecma_op_create_internal_buffer (context_p);
  ...
  ECMA_SET_INTERNAL_VALUE_POINTER (context_p, map_obj_p->u.cls.u3.value, container_p);

  ecma_value_t set_value = ecma_make_object_value (context_p, object_p);
  ...
  result = ecma_op_object_get_by_magic_id (context_p, object_p, adder_string_id);
  ...
  if (!ecma_op_is_callable (context_p, result))
  {
    ...
    result = ecma_raise_type_error (context_p, ECMA_ERR_FUNCTION_ADD_ORSET_IS_NOT_CALLABLE);
    goto cleanup_object;
  }

  ecma_object_t *adder_func_p = ecma_get_object_from_value (context_p, result);

  ecma_value_t next_method;
  result = ecma_op_get_iterator (context_p, iterable, ECMA_VALUE_SYNC_ITERATOR, &next_method);
  ...
  while (true)
  {
    ...
    if (lit_id == LIT_MAGIC_STRING_SET_UL || lit_id == LIT_MAGIC_STRING_WEAKSET_UL)
    {
      ...
      result = ecma_op_function_call (context_p, adder_func_p, set_value, arguments, 1);
    ...
    else
    {
      if (!ecma_is_value_object (result))
      {
        ...
        result = ecma_op_iterator_close (context_p, iterator);
      ...
      result = ecma_op_function_call (context_p, adder_func_p, set_value, arguments, 2);
  ...
  result = ecma_make_object_value (context_p, object_p);
...
ecma_op_container_get_object (ecma_context_t *context_p, /**< JJS context */
                              ecma_value_t this_arg, /**< this argument */
                              lit_magic_string_id_t lit_id) /**< internal class id */
{
  if (ecma_is_value_object (this_arg))
  {
    ecma_object_t *map_object_p = ecma_get_object_from_value (context_p, this_arg);
  ...
  ecma_raise_standard_error_with_format (context_p,
                                         JJS_ERROR_TYPE,
                                         "Expected a % object",
                                         ecma_make_string_value (context_p, ecma_get_magic_string (lit_id)));
 ...
 */
ecma_value_t
ecma_op_container_size (ecma_context_t *context_p, /**< JJS context */
                        ecma_extended_object_t *map_object_p) /**< internal class id */
{
  ecma_collection_t *container_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_collection_t, map_object_p->u.cls.u3.value);

  return ecma_make_uint32_value (context_p, ECMA_CONTAINER_GET_SIZE (container_p));
 ...
 */
ecma_value_t
ecma_op_container_get (ecma_context_t *context_p, /**< JJS context */
                       ecma_extended_object_t *map_object_p, /**< map object */
                       ecma_value_t key_arg, /**< key argument */
                       lit_magic_string_id_t lit_id) /**< internal class id */
{
  ...
  ecma_value_t *entry_p = ecma_op_internal_buffer_find (context_p, container_p, key_arg, lit_id);
  ...
  return ecma_copy_value (context_p, ((ecma_container_pair_t *) entry_p)->value);
 ...
 */
ecma_value_t
ecma_op_container_has (ecma_context_t *context_p, /**< JJS context */
                       ecma_extended_object_t *map_object_p, /**< map object */
                       ecma_value_t key_arg, /**< key argument */
                       lit_magic_string_id_t lit_id) /**< internal class id */
{
 ...
 */
static ecma_value_t
ecma_op_container_set_noramlize_zero (ecma_context_t *context_p, /**< JJS context */
                                      ecma_value_t this_arg) /**< this arg */
{
  if (ecma_is_value_number (this_arg))
  {
    ecma_number_t number_value = ecma_get_number_from_value (context_p, this_arg);

    if (JJS_UNLIKELY (ecma_number_is_zero (number_value) && ecma_number_is_negative (number_value)))
    {
 ...
 */
ecma_value_t
ecma_op_container_set (ecma_context_t *context_p, /**< JJS context */
                       ecma_extended_object_t *map_object_p, /**< map object */
                       ecma_value_t key_arg, /**< key argument */
                       ecma_value_t value_arg, /**< value argument */
                       lit_magic_string_id_t lit_id) /**< internal class id */
{
  ...
  if ((map_object_p->u.cls.u1.container_flags & ECMA_CONTAINER_FLAGS_WEAK) != 0 && !ecma_is_value_object (key_arg))
  {
    return ecma_raise_type_error (context_p, ECMA_ERR_KEY_MUST_BE_AN_OBJECT);
  }
  ...
  if (entry_p == NULL)
  {
    ecma_op_internal_buffer_append (context_p, container_p, ecma_op_container_set_noramlize_zero (context_p, key_arg), value_arg, lit_id);

    if ((map_object_p->u.cls.u1.container_flags & ECMA_CONTAINER_FLAGS_WEAK) != 0)
    {
      ecma_object_t *key_p = ecma_get_object_from_value (context_p, key_arg);
      ecma_op_object_set_weak (context_p, key_p, (ecma_object_t *) map_object_p);
    }
  }
  else
  {
    ecma_op_internal_buffer_update (context_p, entry_p, ecma_op_container_set_noramlize_zero (context_p, value_arg), lit_id);
  }

  ecma_ref_object ((ecma_object_t *) map_object_p);
  return ecma_make_object_value (context_p, (ecma_object_t *) map_object_p);
 ...
 */
ecma_value_t
ecma_op_container_foreach (ecma_context_t *context_p, /**< JJS context */
                           ecma_extended_object_t *map_object_p, /**< map object */
                           ...
                           lit_magic_string_id_t lit_id) /**< internal class id */
{
  ...
  for (uint32_t i = 0; i < ECMA_CONTAINER_ENTRY_COUNT (container_p); i += entry_size)
  {
    ...
    ecma_value_t this_arg = ecma_make_object_value (context_p, (ecma_object_t *) map_object_p);
 ...
 */
ecma_value_t
ecma_op_container_clear (ecma_context_t *context_p, /**< JJS context */
                         ecma_extended_object_t *map_object_p) /**< this argument */
{
  ecma_op_container_free_entries (context_p, (ecma_object_t *) map_object_p);
 ...
 */
ecma_value_t
ecma_op_container_delete (ecma_context_t *context_p, /**< JJS context */
                          ecma_extended_object_t *map_object_p, /**< map object */
                          ecma_value_t key_arg, /**< key argument */
                          lit_magic_string_id_t lit_id) /**< internal class id */
{
  ...
  ecma_op_internal_buffer_delete (context_p, container_p, (ecma_container_pair_t *) entry_p, lit_id);
 ...
 */
ecma_value_t
ecma_op_container_delete_weak (ecma_context_t *context_p, /**< JJS context */
                               ecma_extended_object_t *map_object_p, /**< map object */
                               ecma_value_t key_arg, /**< key argument */
                               lit_magic_string_id_t lit_id) /**< internal class id */
{
  ...
  ecma_object_t *key_object_p = ecma_get_object_from_value (context_p, key_arg);
  ecma_op_object_unref_weak (context_p, key_object_p, ecma_make_object_value (context_p, (ecma_object_t *) map_object_p));
 ...
 */
ecma_value_t
ecma_op_container_find_weak_value (ecma_context_t *context_p, /**< JJS context */
                                   ecma_object_t *object_p, /**< internal container object */
                                   ecma_value_t key_arg) /**< key */
{
  ecma_extended_object_t *map_object_p = (ecma_extended_object_t *) object_p;

  JJS_ASSERT (map_object_p->u.cls.type == ECMA_OBJECT_CLASS_CONTAINER
                && map_object_p->u.cls.u2.container_id == LIT_MAGIC_STRING_WEAKMAP_UL);

  ecma_collection_t *container_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_collection_t, map_object_p->u.cls.u3.value);

  ecma_value_t *entry_p = ecma_op_internal_buffer_find (context_p, container_p, key_arg, map_object_p->u.cls.u2.container_id);
 ...
 */
void
ecma_op_container_remove_weak_entry (ecma_context_t *context_p, /**< JJS context */
                                     ecma_object_t *object_p, /**< internal container object */
                                     ecma_value_t key_arg) /**< key */
{
  ecma_extended_object_t *map_object_p = (ecma_extended_object_t *) object_p;
  ...
  ecma_op_internal_buffer_delete (context_p, container_p, (ecma_container_pair_t *) entry_p, map_object_p->u.cls.u2.container_id);
...
ecma_op_container_create_iterator (ecma_context_t *context_p, /**< JJS context */
                                   ecma_value_t this_arg, /**< this argument */
                                   ecma_builtin_id_t proto_id, /**< prototype builtin id */
 ...
 */
static uint32_t
ecma_op_iterator_get_index (ecma_context_t *context_p, /**< JJS context */
                            ecma_object_t *iter_obj_p) /**< iterator object pointer */
{
  uint32_t index = ((ecma_extended_object_t *) iter_obj_p)->u.cls.u2.iterator_index;

  if (JJS_UNLIKELY (index == ECMA_ITERATOR_INDEX_LIMIT))
  {
    ecma_string_t *prop_name_p = ecma_get_magic_string (LIT_INTERNAL_MAGIC_STRING_ITERATOR_NEXT_INDEX);
    ecma_property_t *property_p = ecma_find_named_property (context_p, iter_obj_p, prop_name_p);
    ecma_property_value_t *value_p = ECMA_PROPERTY_VALUE_PTR (property_p);

    return (uint32_t) (ecma_get_number_from_value (context_p, value_p->value));
  }
 ...
 */
static void
ecma_op_iterator_set_index (ecma_context_t *context_p, /**< JJS context */
                            ecma_object_t *iter_obj_p, /**< iterator object pointer */
                            uint32_t index) /* iterator index to set */
{
  if (JJS_UNLIKELY (index >= ECMA_ITERATOR_INDEX_LIMIT))
  {
    ...
    if (property_p == NULL)
    {
      value_p = ecma_create_named_data_property (context_p, iter_obj_p, prop_name_p, ECMA_PROPERTY_FLAG_WRITABLE, &property_p);
      value_p->value = ecma_make_uint32_value (context_p, index);
    }
    else
    {
 ...
 */
ecma_value_t
ecma_op_container_iterator_next (ecma_context_t *context_p, /**< JJS context */
                                 ...
                                 ecma_object_class_type_t iterator_type) /**< type of the iterator */
{
  ...
  ecma_collection_t *container_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_collection_t, map_object_p->u.cls.u3.value);
  uint32_t entry_count = ECMA_CONTAINER_ENTRY_COUNT (container_p);
  uint32_t index = ecma_op_iterator_get_index (context_p, obj_p);
  ...
  for (uint32_t i = index; i < entry_count; i += entry_size)
  {
    ...
    if (ecma_is_value_empty (*entry_p))
    {
      ...
      {
        ret_value = ecma_create_iter_result_object (context_p, ECMA_VALUE_UNDEFINED, ECMA_VALUE_TRUE);
        break;
      }
    ...
    ecma_op_iterator_set_index (context_p, obj_p, i + entry_size);
    ...
    {
      ret_value = ecma_create_iter_result_object (context_p, key_arg, ECMA_VALUE_FALSE);
    }
    else if (iterator_kind == ECMA_ITERATOR_VALUES)
    {
      ret_value = ecma_create_iter_result_object (context_p, value_arg, ECMA_VALUE_FALSE);
    }
    else
    {
      JJS_ASSERT (iterator_kind == ECMA_ITERATOR_ENTRIES);

      ecma_value_t entry_array_value;
      entry_array_value = ecma_create_array_from_iter_element (context_p, value_arg, key_arg);

      ret_value = ecma_create_iter_result_object (context_p, entry_array_value, ECMA_VALUE_FALSE);
      ecma_free_value (context_p, entry_array_value);
    }
 ...
 */
ecma_value_t
ecma_builtin_container_dispatch_routine (ecma_context_t *context_p, /**< JJS context */
                                         ...
                                         lit_magic_string_id_t lit_id) /**< internal class id */
{
  ecma_extended_object_t *map_object_p = ecma_op_container_get_object (context_p, this_arg, lit_id);
  ...
  switch (builtin_routine_id)
  {
    ...
    {
      return ecma_op_container_delete (context_p, map_object_p, arguments_list_p[0], lit_id);
    }
    case ECMA_CONTAINER_ROUTINE_DELETE_WEAK:
    {
      return ecma_op_container_delete_weak (context_p, map_object_p, arguments_list_p[0], lit_id);
    }
    case ECMA_CONTAINER_ROUTINE_GET:
    {
      return ecma_op_container_get (context_p, map_object_p, arguments_list_p[0], lit_id);
    }
    case ECMA_CONTAINER_ROUTINE_SET:
    {
      return ecma_op_container_set (context_p, map_object_p, arguments_list_p[0], arguments_list_p[1], lit_id);
    }
    case ECMA_CONTAINER_ROUTINE_HAS:
    {
      return ecma_op_container_has (context_p, map_object_p, arguments_list_p[0], lit_id);
    }
    case ECMA_CONTAINER_ROUTINE_FOREACH:
    {
      return ecma_op_container_foreach (context_p, map_object_p, arguments_list_p[0], arguments_list_p[1], lit_id);
    }
    case ECMA_CONTAINER_ROUTINE_SIZE_GETTER:
    {
      return ecma_op_container_size (context_p, map_object_p);
    }
    case ECMA_CONTAINER_ROUTINE_ADD:
    {
      return ecma_op_container_set (context_p, map_object_p, arguments_list_p[0], arguments_list_p[0], lit_id);
    }
    case ECMA_CONTAINER_ROUTINE_CLEAR:
    {
      return ecma_op_container_clear (context_p, map_object_p);
    }
    ...
    case ECMA_CONTAINER_ROUTINE_ENTRIES:
    {
      ...
      return ecma_op_container_create_iterator (context_p, this_arg, builtin_iterator_prototype, iterator_type, kind);
    }
    default:
    {
      JJS_UNREACHABLE ();
