...
{
  /* 1. */
  JJS_ASSERT (ecma_is_value_boolean (done));
...
{
  /* 1. */
  JJS_ASSERT (iterator_type == ECMA_OBJECT_CLASS_ARRAY_ITERATOR || iterator_type == ECMA_OBJECT_CLASS_SET_ITERATOR
  ...
  JJS_ASSERT (kind < ECMA_ITERATOR__COUNT);
...
{
  JJS_ASSERT (ecma_is_value_object (sync_iterator));
