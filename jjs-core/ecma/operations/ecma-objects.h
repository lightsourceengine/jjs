...
#define ECMA_REJECT_WITH_FORMAT(is_throw, msg, ...) \
  ((is_throw) ? ecma_raise_standard_error_with_format (JJS_ERROR_TYPE, (msg), __VA_ARGS__) : ECMA_VALUE_FALSE)
...
ecma_collection_t *ecma_op_object_own_property_keys (ecma_object_t *obj_p, jjs_property_filter_t filter);
...
bool JJS_ATTR_PURE ecma_op_ordinary_object_is_extensible (ecma_object_t *object_p);
