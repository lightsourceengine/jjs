...
#define JMEM_HEAP_GET_OFFSET_FROM_ADDR(p) ((uint32_t) ((uint8_t *) (p) -JJS_HEAP_CONTEXT (area)))
#define JMEM_HEAP_GET_ADDR_FROM_OFFSET(u) ((jmem_heap_free_t *) (JJS_HEAP_CONTEXT (area) + (u)))
...
jmem_heap_alloc (const size_t size) /**< size of requested block */
{
  ...
  JMEM_VALGRIND_DEFINED_SPACE (&JJS_HEAP_CONTEXT (first), sizeof (jmem_heap_free_t));

  /* Fast path for 8 byte chunks, first region is guaranteed to be sufficient. */
  if (required_size == JMEM_ALIGNMENT && JJS_LIKELY (JJS_HEAP_CONTEXT (first).next_offset != JMEM_HEAP_END_OF_LIST))
  {
    data_space_p = JMEM_HEAP_GET_ADDR_FROM_OFFSET (JJS_HEAP_CONTEXT (first).next_offset);
    JJS_ASSERT (jmem_is_heap_pointer (data_space_p));

    JMEM_VALGRIND_DEFINED_SPACE (data_space_p, sizeof (jmem_heap_free_t));
    JJS_CONTEXT (jmem_heap_allocated_size) += JMEM_ALIGNMENT;
    ...
    {
      JJS_HEAP_CONTEXT (first).next_offset = data_space_p->next_offset;
    }
    else
    {
      ...
      remaining_p = JMEM_HEAP_GET_ADDR_FROM_OFFSET (JJS_HEAP_CONTEXT (first).next_offset) + 1;
      ...
      JJS_HEAP_CONTEXT (first).next_offset = JMEM_HEAP_GET_OFFSET_FROM_ADDR (remaining_p);
    }

    JMEM_VALGRIND_NOACCESS_SPACE (data_space_p, sizeof (jmem_heap_free_t));

    if (JJS_UNLIKELY (data_space_p == JJS_CONTEXT (jmem_heap_list_skip_p)))
    {
      JJS_CONTEXT (jmem_heap_list_skip_p) = JMEM_HEAP_GET_ADDR_FROM_OFFSET (JJS_HEAP_CONTEXT (first).next_offset);
    }
  }
  /* Slow path for larger regions. */
  else
  {
    uint32_t current_offset = JJS_HEAP_CONTEXT (first).next_offset;
    jmem_heap_free_t *prev_p = &JJS_HEAP_CONTEXT (first);
    ...
    while (JJS_LIKELY (current_offset != JMEM_HEAP_END_OF_LIST))
    {
      ...
      JJS_ASSERT (jmem_is_heap_pointer (current_p));
      ...
      if (current_p->size >= required_size)
      {
        ...
        JJS_CONTEXT (jmem_heap_list_skip_p) = prev_p;
...
jmem_heap_find_prev (const jmem_heap_free_t *const block_p) /**< which memory block's predecessor we're looking for */
{
  ...
  if (block_p > JJS_CONTEXT (jmem_heap_list_skip_p))
  {
    prev_p = JJS_CONTEXT (jmem_heap_list_skip_p);
  }
  else
  {
    prev_p = &JJS_HEAP_CONTEXT (first);
  }

  JJS_ASSERT (jmem_is_heap_pointer (block_p));
  ...
  while (prev_p->next_offset < block_offset)
  {
    ...
    JJS_ASSERT (jmem_is_heap_pointer (next_p));
                         ...
                         const size_t new_size) /**< desired new size */
{
  ...
  else
  {
    ...
    JJS_CONTEXT (jmem_heap_allocated_size) -= aligned_old_size;
