...
{
  if (JJS_UNLIKELY (code_point >= LIT_UTF8_4_BYTE_CODE_POINT_MIN))
  {
...
{
  JJS_ASSERT (lit_char_is_hex_digit (c));
                     ...
                     uint32_t lookup) /**< size of lookup */
{
  ...
  if (JJS_UNLIKELY (buf_p + lookup > buf_end_p))
  {
    return UINT32_MAX;
  }
  ...
  JJS_ASSERT (value <= LIT_UTF16_CODE_UNIT_MAX);
  return value;
} /* lit_char_hex_lookup */
                   ...
                   const lit_utf8_byte_t *buffer_end_p) /**< buffer end */
{
  ...
  JJS_ASSERT (lit_char_is_decimal_digit (*current_p));
  ...
  while (current_p < buffer_end_p && lit_char_is_decimal_digit (*current_p))
  {
    ...
    if (JJS_UNLIKELY (value > UINT32_MAX / 10) || JJS_UNLIKELY (new_value < value))
    {
      value = UINT32_MAX;
      continue;
    }
...
#else /* !JJS_UNICODE_CASE_CONVERSION */
  if (builder_p != NULL)
  {
