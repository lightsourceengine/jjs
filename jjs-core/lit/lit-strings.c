...
{
  JJS_ASSERT (code_point > LIT_UTF16_CODE_UNIT_MAX);
...
{
  JJS_ASSERT (code_point > LIT_UTF16_CODE_UNIT_MAX);
  JJS_ASSERT (code_point <= LIT_UNICODE_CODE_POINT_MAX);
                                   ...
                                   lit_utf8_size_t cesu8_buf_size) /**< string size */
{
  ...
  JJS_ASSERT (offset == cesu8_buf_size);
                               ...
                               lit_code_point_t *code_point) /**< [out] code point */
{
  ...
  {
    JJS_ASSERT ((c & LIT_UTF8_4_BYTE_MASK) == LIT_UTF8_4_BYTE_MARKER);
    bytes_count = 4;
    ret = ((lit_code_point_t) (c & LIT_UTF8_LAST_3_BITS_MASK));
  }

  JJS_ASSERT (buf_size >= bytes_count);
                                    ...
                                    ecma_char_t *code_unit) /**< [out] code unit */
{
  ...
  {
    JJS_ASSERT ((c & LIT_UTF8_3_BYTE_MASK) == LIT_UTF8_3_BYTE_MARKER);
    bytes_count = 3;
    ret = ((lit_code_point_t) (c & LIT_UTF8_LAST_4_BITS_MASK));
  }
                                ...
                                lit_code_point_t *code_point) /**< [out] code point */
{
  ...
  JJS_ASSERT (buf_p + size <= buf_end_p);
  ...
  if (lit_is_code_point_utf16_high_surrogate (code_unit))
  {
    ...
    if (buf_p < buf_end_p)
    {
      ...
      {
        JJS_ASSERT (buf_p + next_size <= buf_end_p);
...
{
  JJS_ASSERT (utf8_buf_p != NULL || utf8_buf_size == 0);
                              ...
                              lit_utf8_size_t code_unit_offset) /**< ofset of a code_unit */
{
  ...
  do
  {
    JJS_ASSERT (current_p < utf8_buf_p + utf8_buf_size);
 ...
 */
extern inline lit_utf8_size_t JJS_ATTR_ALWAYS_INLINE
lit_get_unicode_char_size_by_utf8_first_byte (const lit_utf8_byte_t first_byte) /**< buffer with characters */
{
  ...
  {
    JJS_ASSERT ((first_byte & LIT_UTF8_3_BYTE_MASK) == LIT_UTF8_3_BYTE_MARKER);
    return 3;
  }
                                                ...
                                                *   should be at least LIT_UTF8_MAX_BYTES_IN_CODE_UNIT */
{
  ...
  else if (code_unit <= LIT_UTF8_2_BYTE_CODE_POINT_MAX)
  {
    ...
    JJS_ASSERT (first_byte_bits == code_unit_bits);
                                               ...
                                               *   its size should be at least 4 bytes */
{
  ...
  else if (code_point <= LIT_UTF8_2_BYTE_CODE_POINT_MAX)
  {
    ...
    JJS_ASSERT (first_byte_bits == code_point_bits);
  ...
  {
    JJS_ASSERT (code_point <= LIT_UTF8_4_BYTE_CODE_POINT_MAX);
                                         ...
                                         lit_utf8_size_t utf8_size) /**< size of utf-8 buffer */
{
  ...
  JJS_ASSERT (utf8_cursor_p <= utf8_end_p);
...
{
  JJS_ASSERT (lit_is_code_point_utf16_high_surrogate (high_surrogate));
  JJS_ASSERT (lit_is_code_point_utf16_low_surrogate (low_surrogate));
