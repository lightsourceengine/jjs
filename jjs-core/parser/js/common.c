 ...
 */
static void
util_print_chars (ecma_context_t *context_p, /**< JJS context */
                  const uint8_t *char_p, /**< character pointer */
                  size_t size) /**< size */
{
  while (size > 0)
  {
    JJS_DEBUG_MSG (context_p, "%c", *char_p++);
    size--;
  }
} /* util_print_chars */
 ...
 */
static void
util_print_number (ecma_context_t *context_p, /**< JJS context */
                   ecma_number_t num_p) /**< number to print */
{
  lit_utf8_byte_t str_buf[ECMA_MAX_CHARS_IN_STRINGIFIED_NUMBER];
  lit_utf8_size_t str_size = ecma_number_to_utf8_string (num_p, str_buf, sizeof (str_buf));
  str_buf[str_size] = 0;
  JJS_DEBUG_MSG (context_p, "%s", str_buf);
} /* util_print_number */
 ...
 */
static void
util_print_bigint (ecma_context_t *context_p, /**< JJS context */
                   ecma_value_t bigint) /**< bigint to print */
{
  if (bigint == ECMA_BIGINT_ZERO)
  {
    JJS_DEBUG_MSG (context_p, "0");
    return;
  }

  ecma_extended_primitive_t *bigint_p = ecma_get_extended_primitive_from_value (context_p, bigint);
  uint32_t char_start_p, char_size_p;
  lit_utf8_byte_t *string_buffer_p = ecma_big_uint_to_string (context_p, bigint_p, 10, &char_start_p, &char_size_p);

  if (JJS_UNLIKELY (string_buffer_p == NULL))
  {
    JJS_DEBUG_MSG (context_p, "<out-of-memory>");
    return;
  }

  JJS_ASSERT (char_start_p > 0);
  ...
  util_print_chars (context_p, string_buffer_p + char_start_p, char_size_p - char_start_p);
 ...
 */
void
util_print_literal (ecma_context_t *context_p, /**< JJS context */
                    lexer_literal_t *literal_p) /**< literal */
{
  switch (literal_p->type)
  {
    ...
    {
      JJS_DEBUG_MSG (context_p, "ident(");
      util_print_chars (context_p, literal_p->u.char_p, literal_p->prop.length);
      break;
    }
    case LEXER_FUNCTION_LITERAL:
    {
      JJS_DEBUG_MSG (context_p, "function");
      return;
    }
    case LEXER_STRING_LITERAL:
    {
      JJS_DEBUG_MSG (context_p, "string(");
      util_print_chars (context_p, literal_p->u.char_p, literal_p->prop.length);
      break;
    }
    case LEXER_NUMBER_LITERAL:
    {
      ...
      {
        JJS_DEBUG_MSG (context_p, "bigint(");
        util_print_bigint (context_p, literal_p->u.value);
        break;
      }
#endif /* JJS_BUILTIN_BIGINT */
      JJS_DEBUG_MSG (context_p, "number(");
      util_print_number (context_p, ecma_get_number_from_value (context_p, literal_p->u.value));
      break;
    }
    case LEXER_REGEXP_LITERAL:
    {
      JJS_DEBUG_MSG (context_p, "regexp");
      return;
    }
    default:
    {
      JJS_DEBUG_MSG (context_p, "unknown");
      return;
    }
  }
 ...
 */
static void
util_print_literal_value (ecma_context_t *context_p, /**< JJS context */
                          ecma_compiled_code_t *compiled_code_p, /**< compiled code */
                          uint16_t literal_index) /**< literal index */
{
  ...
  {
    JJS_DEBUG_MSG (context_p, " arg:%d", literal_index);
    return;
  }

  if (literal_index < register_end)
  {
    JJS_DEBUG_MSG (context_p, " reg:%d", literal_index);
    return;
  }

  if (literal_index >= const_literal_end)
  {
    JJS_DEBUG_MSG (context_p, " lit:%d", literal_index);
    return;
  }

  if (literal_index < ident_end)
  {
    JJS_DEBUG_MSG (context_p, " ident:%d->", literal_index);
  }
  else
  {
    JJS_DEBUG_MSG (context_p, " const:%d->", literal_index);
  }
  ...
  {
    JJS_DEBUG_MSG (context_p, "number(");
    util_print_number (context_p, ecma_get_number_from_value (context_p, value));
  }
#if JJS_BUILTIN_BIGINT
  else if (ecma_is_value_bigint (value))
  {
    JJS_DEBUG_MSG (context_p, "bigint(");
    util_print_bigint (context_p, value);
  }
#endif /* JJS_BUILTIN_BIGINT */
  else
  {
    ecma_string_t *literal_p = ecma_get_string_from_value (context_p, value);

    JJS_DEBUG_MSG (context_p, "string(");

    ECMA_STRING_TO_UTF8_STRING (context_p, literal_p, chars_p, literal_size);
    util_print_chars (context_p, chars_p, literal_size);
    ECMA_FINALIZE_UTF8_STRING (context_p, chars_p, literal_size);
  }
 ...
 */
void
util_print_cbc (ecma_context_t *context_p, /**< JJS context */
                ecma_compiled_code_t *compiled_code_p) /**< compiled code */
{
  uint8_t flags;
  ...
  switch (CBC_FUNCTION_GET_TYPE (compiled_code_p->status_flags))
  {
    ...
    {
      JJS_DEBUG_MSG (context_p, ",constructor");
      break;
    }
    case CBC_FUNCTION_GENERATOR:
    {
      JJS_DEBUG_MSG (context_p, ",generator");
      break;
    }
    case CBC_FUNCTION_ASYNC:
    {
      JJS_DEBUG_MSG (context_p, ",async");
      break;
    }
    case CBC_FUNCTION_ASYNC_GENERATOR:
    {
      JJS_DEBUG_MSG (context_p, ",async_generator");
      break;
    }
    case CBC_FUNCTION_ACCESSOR:
    {
      JJS_DEBUG_MSG (context_p, ",accessor");
      break;
    }
    case CBC_FUNCTION_ARROW:
    {
      JJS_DEBUG_MSG (context_p, ",arrow");
      break;
    }
    case CBC_FUNCTION_ASYNC_ARROW:
    {
      JJS_DEBUG_MSG (context_p, ",async_arrow");
      break;
    }
  }
  ...
  JJS_DEBUG_MSG (context_p, "  Argument range end: %d\n", (int) argument_end);
  JJS_DEBUG_MSG (context_p, "  Register range end: %d\n", (int) register_end);
  JJS_DEBUG_MSG (context_p, "  Identifier range end: %d\n", (int) ident_end);
  JJS_DEBUG_MSG (context_p, "  Const literal range end: %d\n", (int) const_literal_end);
  JJS_DEBUG_MSG (context_p, "  Literal range end: %d\n\n", (int) literal_end);
  ...
  if (compiled_code_p->status_flags & CBC_CODE_FLAGS_HAS_EXTENDED_INFO)
  {
    ...
    if (extended_info & CBC_EXTENDED_CODE_FLAGS_HAS_ARGUMENT_LENGTH)
    {
      ...
      JJS_DEBUG_MSG (context_p, "  [Extended] Argument length: %d\n", (int) argument_length);
    }
    ...
    if (extended_info & CBC_EXTENDED_CODE_FLAGS_HAS_SOURCE_CODE_RANGE)
    {
      ...
      JJS_DEBUG_MSG (context_p, "  [Extended] Source code range: %d - %d\n", (int) range_start, (int) range_end);
    }
  ...
  while (byte_code_p < byte_code_end_p)
  {
    ...
    if (opcode != CBC_EXT_OPCODE)
    {
      ...
      JJS_DEBUG_MSG (context_p, " %3d : %s", (int) cbc_offset, cbc_names[opcode]);
      byte_code_p++;
    }
    else
    {
      ...
      JJS_DEBUG_MSG (context_p, " %3d : %s", (int) cbc_offset, cbc_ext_names[ext_opcode]);
      byte_code_p += 2;
    }
    ...
    if (flags & (CBC_HAS_LITERAL_ARG | CBC_HAS_LITERAL_ARG2))
    {
      ...
      util_print_literal_value (context_p, compiled_code_p, literal_index);
    }
    ...
    if (flags & CBC_HAS_BYTE_ARG)
    {
      ...
      {
        JJS_DEBUG_MSG (context_p, " number:%d", (int) *byte_code_p + 1);
      }
      ...
      {
        JJS_DEBUG_MSG (context_p, " byte_arg:%d", *byte_code_p);
      }
      byte_code_p++;
    }
    ...
    if (flags & CBC_HAS_BRANCH_ARG)
    {
      ...
      JJS_DEBUG_MSG (context_p, " offset:%d(->%d)",
                       (int) offset,
