...
util_print_literal (lexer_literal_t *literal_p) /**< literal */
{
  switch (literal_p->type)
  {
    ...
    {
      JJS_DEBUG_MSG ("function");
      return;
    }
    case LEXER_STRING_LITERAL:
    {
      JJS_DEBUG_MSG ("string(");
      util_print_chars (literal_p->u.char_p, literal_p->prop.length);
      break;
    }
                          ...
                          uint16_t literal_index) /**< literal index */
{
  ...
  {
    JJS_DEBUG_MSG (" arg:%d", literal_index);
    return;
  }

  if (literal_index < register_end)
  {
    JJS_DEBUG_MSG (" reg:%d", literal_index);
    return;
  }

  if (literal_index >= const_literal_end)
  {
    JJS_DEBUG_MSG (" lit:%d", literal_index);
    return;
  }

  if (literal_index < ident_end)
  {
    JJS_DEBUG_MSG (" ident:%d->", literal_index);
  }
  else
  {
    JJS_DEBUG_MSG (" const:%d->", literal_index);
  }
...
util_print_cbc (ecma_compiled_code_t *compiled_code_p) /**< compiled code */
{
  ...
  switch (CBC_FUNCTION_GET_TYPE (compiled_code_p->status_flags))
  {
    ...
    {
      JJS_DEBUG_MSG (",constructor");
      break;
    }
    case CBC_FUNCTION_GENERATOR:
    {
      JJS_DEBUG_MSG (",generator");
      break;
    }
    ...
    {
      JJS_DEBUG_MSG (",async_generator");
      break;
    }
    case CBC_FUNCTION_ACCESSOR:
    {
      JJS_DEBUG_MSG (",accessor");
      break;
    }
  ...
  JJS_DEBUG_MSG ("  Argument range end: %d\n", (int) argument_end);
  JJS_DEBUG_MSG ("  Register range end: %d\n", (int) register_end);
  JJS_DEBUG_MSG ("  Identifier range end: %d\n", (int) ident_end);
  JJS_DEBUG_MSG ("  Const literal range end: %d\n", (int) const_literal_end);
  JJS_DEBUG_MSG ("  Literal range end: %d\n\n", (int) literal_end);
  ...
  if (compiled_code_p->status_flags & CBC_CODE_FLAGS_HAS_EXTENDED_INFO)
  {
    ...
    if (extended_info & CBC_EXTENDED_CODE_FLAGS_HAS_ARGUMENT_LENGTH)
    {
      ...
      JJS_DEBUG_MSG ("  [Extended] Argument length: %d\n", (int) argument_length);
    }
    ...
    if (extended_info & CBC_EXTENDED_CODE_FLAGS_HAS_SOURCE_CODE_RANGE)
    {
      ...
      JJS_DEBUG_MSG ("  [Extended] Source code range: %d - %d\n", (int) range_start, (int) range_end);
    }
  ...
  while (byte_code_p < byte_code_end_p)
  {
    ...
    if (opcode != CBC_EXT_OPCODE)
    {
      ...
      JJS_DEBUG_MSG (" %3d : %s", (int) cbc_offset, cbc_names[opcode]);
      byte_code_p++;
    }
    else
    {
      ...
      JJS_DEBUG_MSG (" %3d : %s", (int) cbc_offset, cbc_ext_names[ext_opcode]);
      byte_code_p += 2;
    }
    ...
    if (flags & CBC_HAS_BYTE_ARG)
    {
      ...
      {
        JJS_DEBUG_MSG (" number:%d", (int) *byte_code_p + 1);
      }
      ...
      {
        JJS_DEBUG_MSG (" byte_arg:%d", *byte_code_p);
      }
      byte_code_p++;
    }
