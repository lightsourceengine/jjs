...
JJS_STATIC_ASSERT (sizeof (parser_binary_precedence_table) == 42,
...
static void parser_parse_postfix_expression (parser_context_t *context_p, size_t *grouping_level_p);
...
parser_push_result (parser_context_t *context_p) /**< context */
{
  ...
  {
    JJS_ASSERT (CBC_SAME_ARGS (context_p->last_cbc_opcode, context_p->last_cbc_opcode + 1));
        ...
        && context_p->stack_depth >= context_p->stack_limit)
    {
      ...
      JJS_ASSERT (context_p->stack_depth == context_p->stack_limit);
...
{
  JJS_ASSERT (context_p->last_cbc.literal_type == LEXER_IDENT_LITERAL);

  if (JJS_UNLIKELY (context_p->status_flags & PARSER_IS_STRICT))
  {
                             ...
                             uint16_t opcode) /* opcode */
{
  ...
  {
    JJS_ASSERT (context_p->last_cbc_opcode == CBC_PUSH_THREE_LITERALS);
                                 ...
                                 cbc_opcode_t opcode) /**< opcode */
{
      ...
      && context_p->last_cbc.literal_type == LEXER_IDENT_LITERAL)
  {
    ...
    {
      JJS_ASSERT (CBC_SAME_ARGS (CBC_PUSH_LITERAL, opcode + CBC_UNARY_LVALUE_WITH_IDENT));
  ...
  {
    JJS_ASSERT (CBC_SAME_ARGS (CBC_PUSH_PROP, opcode));
                                       ...
                                       uint8_t opts) /**< options */
{
  ...
  JJS_ASSERT (context_p->private_context_p);
...
{
  JJS_ASSERT (context_p->token.type == LEXER_LEFT_BRACE);
  ...
  while (true)
  {
    ...
    {
      JJS_ASSERT (!is_static);
      JJS_ASSERT (opts & PARSER_CLASS_LITERAL_CTOR_PRESENT);
      JJS_ASSERT (ctor_literal_p != NULL);
    ...
    if (context_p->token.type == LEXER_PROPERTY_GETTER || context_p->token.type == LEXER_PROPERTY_SETTER)
    {
      ...
      {
        JJS_ASSERT (!is_static || is_private);
    ...
    if (!(status_flags & (PARSER_IS_ASYNC_FUNCTION | PARSER_IS_GENERATOR_FUNCTION)))
    {
      if (is_static_block || !lexer_check_next_character (context_p, LIT_CHAR_LEFT_PAREN))
      {
        ...
        if (is_static_block)
        {
          ...
          JJS_ASSERT (context_p->next_scanner_info_p->type == SCANNER_TYPE_CLASS_STATIC_BLOCK_END);
        ...
        else if (lexer_consume_assign (context_p))
        {
          ...
          JJS_ASSERT (context_p->next_scanner_info_p->type == SCANNER_TYPE_CLASS_FIELD_INITIALIZER_END);
      ...
      if (!is_computed)
      {
        ...
        {
          JJS_ASSERT (context_p->token.lit_location.type == LEXER_IDENT_LITERAL
                               ...
                               size_t *grouping_level_p) /**< grouping level */
{
  ...
  switch (context_p->token.type)
  {
    ...
    case LEXER_LITERAL:
    {
      if (JJS_UNLIKELY (context_p->next_scanner_info_p->source_p == context_p->source_p))
      {
        JJS_ASSERT (context_p->next_scanner_info_p->type == SCANNER_TYPE_FUNCTION);
        ...
        {
          JJS_ASSERT (lexer_token_is_async (context_p));
          JJS_ASSERT (!(context_p->next_scanner_info_p->u8_arg & SCANNER_FUNCTION_STATEMENT));
      ...
      else if (type == LEXER_NUMBER_LITERAL)
      {
            ...
            && !lexer_check_post_primary_exp (context_p))
        {
          do
          {
            ...
            else if (JJS_LIKELY (context_p->token.extra_value == LEXER_NUMBER_BIGINT))
            {
              break;
            }
        ...
        {
          JJS_ASSERT (context_p->lit_object.index <= CBC_PUSH_NUMBER_BYTE_RANGE_END);
    ...
    {
      JJS_ASSERT ((context_p->status_flags & PARSER_IS_GENERATOR_FUNCTION)
    ...
    case LEXER_KEYW_IMPORT:
    {
      ...
      if (context_p->token.type == LEXER_DOT)
      {
        ...
        JJS_ASSERT (context_p->global_status_flags & ECMA_PARSE_INTERNAL_HAS_IMPORT_META);
...
{
  JJS_ASSERT (context_p->token.type == LEXER_DOT || context_p->token.type == LEXER_QUESTION_MARK_DOT);
  ...
  switch (context_p->last_cbc_opcode)
  {
    ...
    {
      JJS_ASSERT (CBC_ARGS_EQ (CBC_PUSH_PROP_LITERAL_LITERAL, CBC_HAS_LITERAL_ARG | CBC_HAS_LITERAL_ARG2));
...
{
  JJS_ASSERT (context_p->last_cbc_opcode == PARSER_TO_EXT_OPCODE (CBC_EXT_PUSH_SUPER));
  context_p->last_cbc_opcode = PARSER_CBC_UNAVAILABLE;
...
parser_form_call_reference (parser_context_t *context_p) /**< context */
{
  ...
  switch (context_p->last_cbc_opcode)
  {
    ...
    {
      JJS_ASSERT (PARSER_IS_PUSH_LITERALS_WITH_THIS (context_p->last_cbc_opcode));

      if (JJS_UNLIKELY (context_p->status_flags & PARSER_INSIDE_WITH)
          && context_p->last_cbc.literal_type == LEXER_IDENT_LITERAL)
      {
    ...
    {
      JJS_ASSERT (PARSER_IS_PUSH_PROP (context_p->last_cbc_opcode));
...
parser_emit_call_01 (parser_context_t *context_p, /**< context */
                     parser_call_reference_t *call_ref_p) /**< call reference */
{
  JJS_ASSERT (call_ref_p->call_arguments <= 1);
...
parser_emit_call_2 (parser_context_t *context_p, /**< context */
                    parser_call_reference_t *call_ref_p) /**< call reference */
{
  JJS_ASSERT (call_ref_p->call_arguments == 2);
  ...
  switch (call_ref_p->opcode)
  {
    ...
    case CBC_CALL_PROP:
    {
      ...
      JJS_ASSERT (context_p->stack_depth > 0);
...
parser_parse_call_expression (parser_context_t *context_p, /**< context */
                              parser_call_reference_t *call_ref_p) /**< call reference */
{
  JJS_ASSERT (context_p->token.type == LEXER_LEFT_PAREN || context_p->token.type == LEXER_TEMPLATE_LITERAL);
...
parser_parse_optional_chain (parser_context_t *context_p, /**< context */
                             size_t *grouping_level_p) /**< grouping level */
{
  JJS_ASSERT (context_p->token.type == LEXER_QUESTION_MARK_DOT);
  ...
  switch (lexer_peek_next_character (context_p))
  {
    ...
    case LIT_CHAR_LEFT_PAREN:
    {
      ...
      if (call_ref.opcode == CBC_CALL_PROP)
      {
        ...
        parser_flush_cbc (context_p);
        JJS_ASSERT (context_p->stack_limit - context_p->stack_depth >= 2);
...
{
  JJS_ASSERT (context_p->stack_top_uint8 == LEXER_KEYW_NEW);
...
parser_process_prefix_unary_opcodes (parser_context_t *context_p) /**< context */
{
  ...
  while (true)
  {
    ...
    else if (JJS_UNLIKELY (token == LEXER_KEYW_AWAIT))
    {
      cbc_ext_opcode_t opcode =
    ...
    else
    {
      ...
      else
      {
        if (context_p->last_cbc_opcode == CBC_PUSH_LITERAL)
        {
          ...
          JJS_ASSERT (CBC_SAME_ARGS (context_p->last_cbc_opcode, token + 1));
 ...
 */
static void
parser_parse_postfix_expression (parser_context_t *context_p, /**< context */
                                 size_t *grouping_level_p) /**< grouping level */
{
...
} /* parser_parse_postfix_expression */
                                              ...
                                              uint32_t pattern_flags) /**< pattern flags */
{
      ...
      && context_p->last_cbc.literal_type == LEXER_IDENT_LITERAL)
  {
    ...
    JJS_ASSERT (CBC_SAME_ARGS (CBC_PUSH_LITERAL, assign_opcode));
  }
  else if (context_p->last_cbc_opcode == CBC_PUSH_PROP)
  {
    JJS_ASSERT (CBC_SAME_ARGS (CBC_PUSH_PROP, CBC_ASSIGN));
    context_p->last_cbc_opcode = PARSER_CBC_UNAVAILABLE;
  }
  else if (context_p->last_cbc_opcode == CBC_PUSH_PROP_LITERAL)
  {
    ...
    {
      JJS_ASSERT (CBC_SAME_ARGS (CBC_PUSH_PROP_LITERAL, CBC_ASSIGN_PROP_LITERAL));
  ...
  {
    JJS_ASSERT (CBC_SAME_ARGS (CBC_PUSH_PROP_LITERAL_LITERAL, CBC_PUSH_TWO_LITERALS));
    context_p->last_cbc_opcode = CBC_PUSH_TWO_LITERALS;
  }
  else if (context_p->last_cbc_opcode == CBC_PUSH_PROP_THIS_LITERAL)
  {
    if (context_p->last_cbc.literal_type != LEXER_IDENT_LITERAL)
    {
      JJS_ASSERT (CBC_SAME_ARGS (CBC_PUSH_PROP_THIS_LITERAL, CBC_ASSIGN_PROP_THIS_LITERAL));
      parser_stack_push_uint16 (context_p, context_p->last_cbc.literal_index);
...
{
  JJS_ASSERT (LEXER_IS_BINARY_OP_TOKEN (context_p->token.type));
  parser_push_result (context_p);
                                        ...
                                        uint8_t token) /**< token */
{
  ...
  if (JJS_UNLIKELY (opcode == CBC_EXT_OPCODE))
  {
    parser_stack_pop_uint8 (context_p);
    JJS_ASSERT (context_p->stack_top_uint8 == CBC_EXT_ASSIGN_SUPER
  ...
  else
  {
    ...
    {
      JJS_ASSERT (opcode == CBC_ASSIGN_SET_IDENT || opcode == CBC_ASSIGN_PROP_LITERAL
  ...
  if (JJS_UNLIKELY (context_p->stack_top_uint8 == LEXER_ASSIGN_GROUP_EXPR))
  {
    group_expr_assingment = true;
    parser_stack_pop_uint8 (context_p);
  }

  if (JJS_UNLIKELY (context_p->stack_top_uint8 == LEXER_ASSIGN_CONST))
  {
    parser_stack_pop_uint8 (context_p);
    parser_emit_cbc_ext (context_p, CBC_EXT_THROW_ASSIGN_CONST_ERROR);
  }
  ...
  {
    if (JJS_UNLIKELY (token == LEXER_ASSIGN_REFERENCE))
    {
  ...
  {
    JJS_ASSERT (CBC_ARGS_EQ (CBC_ASSIGN_LITERAL_SET_IDENT, CBC_HAS_LITERAL_ARG | CBC_HAS_LITERAL_ARG2));
...
parser_process_logical_assignment_token (parser_context_t *context_p) /**< context */
{
  ...
  uint8_t token = context_p->stack_top_uint8;
  JJS_ASSERT (token == LEXER_ASSIGN_REFERENCE);
  parser_stack_pop_uint8 (context_p);
                             ...
                             uint8_t token) /**< token */
{
  ...
  if (context_p->last_cbc_opcode == CBC_PUSH_LITERAL)
  {
    JJS_ASSERT (CBC_SAME_ARGS (context_p->last_cbc_opcode, opcode + CBC_BINARY_WITH_LITERAL));
  ...
  if (context_p->last_cbc_opcode == CBC_PUSH_TWO_LITERALS)
  {
    JJS_ASSERT (CBC_ARGS_EQ (opcode + CBC_BINARY_WITH_TWO_LITERALS, CBC_HAS_LITERAL_ARG | CBC_HAS_LITERAL_ARG2));
                           ...
                           parser_pattern_flags_t flags) /**< flags */
{
  ...
  {
    JJS_ASSERT (flags & PARSER_PATTERN_TARGET_ON_STACK);
                         ...
                         parser_pattern_end_marker_t *end_marker_p) /**< pattern end position  */
{
  ...
  {
    JJS_ASSERT (!(flags & PARSER_PATTERN_TARGET_DEFAULT));
    lexer_next_token (context_p);
  }
...
parser_pattern_process_nested_pattern (parser_context_t *context_p, /**< context */
                                       ...
                                       uint16_t literal_index) /**< literal index for object pattern */
{
  JJS_ASSERT (context_p->token.type == LEXER_LEFT_BRACE || context_p->token.type == LEXER_LEFT_SQUARE);
                                 ...
                                 parser_pattern_flags_t flags) /**< flags */
{
  ...
  while (true)
  {
    ...
    else
    {
      ...
      lexer_next_token (context_p);
      JJS_ASSERT (context_p->token.type == LEXER_RIGHT_BRACE || context_p->token.type == LEXER_ASSIGN
                    || context_p->token.type == LEXER_COMMA);
  ...
  JJS_ASSERT (!!(flags & PARSER_PATTERN_HAS_REST_ELEMENT) == rest_found);
#endif /* !defined(JJS_NDEBUG) */
...
{
  JJS_ASSERT (context_p->token.type == LEXER_QUESTION_MARK);
                                 ...
                                 size_t *grouping_level_p) /**< grouping level */
{
  ...
  JJS_ASSERT (*grouping_level_p >= PARSER_GROUPING_LEVEL_INCREASE);
  (*grouping_level_p) -= PARSER_GROUPING_LEVEL_INCREASE;
  ...
  if (JJS_UNLIKELY (context_p->token.type == LEXER_ASSIGN
...
JJS_STATIC_ASSERT (PARSE_EXPR_LEFT_HAND_SIDE == 0x1, value_of_parse_expr_left_hand_side_must_be_1);
