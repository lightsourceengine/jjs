...
parser_line_info_encode_small (uint8_t *buffer_p, /**< target buffer */
                               uint32_t value) /**< encoded value */
{
  if (JJS_LIKELY (value < ECMA_LINE_INFO_ENCODE_TWO_BYTE_MIN))
  {
    buffer_p[0] = (uint8_t) value;
    return 1;
  }

  if (JJS_LIKELY (value < ECMA_LINE_INFO_ENCODE_VLQ_MIN))
  {
...
parser_line_info_iterator_get (parser_line_info_iterator_t *iterator_p) /**< iterator */
{
  ...
  JJS_ASSERT (iterator_p->offset <= iterator_p->current_page_p->bytes[0]);
...
parser_line_info_generate (parser_context_t *parser_context_p) /**< context */
{
  ...
  while (true)
  {
    ...
    do
    {
      ...
      {
        value = parser_line_info_iterator_get (&iterator);
        JJS_ASSERT (value != ((0 << 1) | ECMA_LINE_INFO_DECREASE));
      ...
      if (block_size_p != NULL)
      {
        ...
        JJS_ASSERT (stream_size > ECMA_LINE_INFO_STREAM_SIZE_MIN
