...
JJS_STATIC_ASSERT (PARSER_MAXIMUM_NUMBER_OF_LITERALS + PARSER_MAXIMUM_NUMBER_OF_REGISTERS < PARSER_REGISTER_START,
                     maximum_number_of_literals_plus_registers_must_be_less_than_register_start);

JJS_STATIC_ASSERT ((SCANNER_LITERAL_IS_ARROW_DESTRUCTURED_ARG & SCANNER_LITERAL_IS_LOCAL) == 0,
                     is_arrow_arg_binding_flag_must_not_use_local_flags);

JJS_STATIC_ASSERT ((SCANNER_LITERAL_IS_LET & SCANNER_LITERAL_IS_LOCAL) != 0, is_let_flag_must_use_local_flags);

JJS_STATIC_ASSERT ((SCANNER_LITERAL_IS_CONST & SCANNER_LITERAL_IS_LOCAL) != 0, is_const_flag_must_use_local_flags);

JJS_STATIC_ASSERT ((SCANNER_LITERAL_IS_FUNC_DECLARATION & SCANNER_LITERAL_IS_LOCAL) != 0,
                     is_func_declaration_flag_must_use_local_flags);

JJS_STATIC_ASSERT ((SCANNER_LITERAL_IS_DESTRUCTURED_ARG & SCANNER_LITERAL_IS_LOCAL) != 0,
                     is_arg_binding_flag_must_use_local_flags);

JJS_STATIC_ASSERT (SCANNER_LITERAL_IS_FUNC_DECLARATION != SCANNER_LITERAL_IS_DESTRUCTURED_ARG,
                     is_func_declaration_must_be_different_from_is_arg_binding);

JJS_STATIC_ASSERT (PARSER_SCOPE_STACK_IS_CONST_REG == PARSER_SCOPE_STACK_IS_LOCAL_CREATED,
                         ...
                         size_t size) /**< size excluding the stream */
{
  ...
  while (data_p[0] != SCANNER_STREAM_TYPE_END)
  {
    switch (data_p[0] & SCANNER_STREAM_TYPE_MASK)
    {
      ...
      default:
      {
        JJS_ASSERT ((data_p[0] & SCANNER_STREAM_TYPE_MASK) == SCANNER_STREAM_TYPE_HOLE
                     ...
                     size_t size) /**< size of the memory block */
{
  ...
  JJS_ASSERT (scanner_info_p != NULL);
  ...
  JJS_ASSERT (source_p != scanner_info_p->source_p);

  new_scanner_info_p->next_p = scanner_info_p;

  if (JJS_LIKELY (prev_scanner_info_p == NULL))
  {
                           ...
                           uint16_t status_flags) /**< combination of scanner_literal_pool_flags_t flags */
{
  ...
  {
    JJS_ASSERT (prev_literal_pool_p != NULL);
  ...
  if (prev_literal_pool_p != NULL)
  {
    ...
    JJS_ASSERT (!(status_flags & SCANNER_LITERAL_POOL_IS_STRICT) == !(context_p->status_flags & PARSER_IS_STRICT));
  }
...
JJS_STATIC_ASSERT (PARSER_MAXIMUM_IDENT_LENGTH <= UINT8_MAX, maximum_ident_length_must_fit_in_a_byte);
                          ...
                          scanner_context_t *scanner_context_p) /**< scanner context */
{
  ...
  if (JJS_UNLIKELY (literal_pool_p->source_p == NULL))
  {
    JJS_ASSERT (literal_pool_p->status_flags & SCANNER_LITERAL_POOL_FUNCTION);
    JJS_ASSERT (literal_pool_p->literal_pool.data.first_p == NULL
  ...
  while ((literal_p = (lexer_lit_location_t *) parser_list_iterator_next (&literal_iterator)) != NULL)
  {
    ...
    if (JJS_UNLIKELY (no_declarations > PARSER_MAXIMUM_DEPTH_OF_SCOPE_STACK))
    {
      continue;
    }
    ...
    {
      JJS_ASSERT (arguments_type != SCANNER_ARGUMENTS_NOT_PRESENT);
      status_flags |= SCANNER_LITERAL_POOL_NO_ARGUMENTS;

      if (type & SCANNER_LITERAL_IS_ARG)
      {
        JJS_ASSERT (arguments_type != SCANNER_ARGUMENTS_PRESENT
                      && arguments_type != SCANNER_ARGUMENTS_PRESENT_NO_REG);
        arguments_type = SCANNER_ARGUMENTS_NOT_PRESENT;
    ...
    {
      JJS_ASSERT ((status_flags & SCANNER_LITERAL_POOL_FUNCTION) || !(literal_p->type & SCANNER_LITERAL_IS_ARG));
      ...
      if (type & SCANNER_LITERAL_IS_FUNC)
      {
        ...
        {
          JJS_ASSERT (type & SCANNER_LITERAL_IS_LET);
    ...
    if (prev_literal_pool_p != NULL && literal_p->length > 0)
    {
      ...
      JJS_ASSERT (type == 0 || !(status_flags & SCANNER_LITERAL_POOL_FUNCTION));
  ...
  if ((status_flags & SCANNER_LITERAL_POOL_FUNCTION) || (compressed_size > 1))
  {
    ...
    while ((literal_p = (lexer_lit_location_t *) parser_list_iterator_next (&literal_iterator)) != NULL)
    {
      ...
      else if (literal_p->type & SCANNER_LITERAL_IS_ARG)
      {
        ...
        if (literal_p->type & SCANNER_LITERAL_IS_VAR)
        {
          ...
          JJS_ASSERT (type == SCANNER_STREAM_TYPE_ARG_VAR || type == SCANNER_STREAM_TYPE_DESTRUCTURED_ARG_VAR);
        }
      }
    ...
    JJS_ASSERT (((uint8_t *) info_p) + compressed_size == data_p + 1);
  }
                          ...
                          scanner_context_t *scanner_context_p) /**< scanner context */
{
  ...
  JJS_ASSERT (SCANNER_LITERAL_POOL_MAY_HAVE_ARGUMENTS (literal_pool_p->status_flags));

  if (JJS_UNLIKELY (can_eval))
  {
    if (prev_literal_pool_p != NULL)
    {
      prev_literal_pool_p->status_flags |= SCANNER_LITERAL_POOL_CAN_EVAL;
                            ...
                            const lexer_lit_location_t *literal_location_p) /**< literal */
{
  while (true)
  {
    ...
    if (JJS_LIKELY (!(literal_location_p->status_flags & LEXER_LIT_LOCATION_HAS_ESCAPE)))
    {
      while ((literal_p = (lexer_lit_location_t *) parser_list_iterator_next (&literal_iterator)) != NULL)
      {
    ...
    if (JJS_UNLIKELY (literal_pool_p->status_flags & SCANNER_LITERAL_POOL_CLASS_NAME))
    {
 ...
 * @return pointer to the literal
 */
extern inline lexer_lit_location_t *JJS_ATTR_ALWAYS_INLINE
                         ...
                         scanner_context_t *scanner_context_p) /**< scanner context */
{
  ...
  if (JJS_LIKELY (!(context_p->token.lit_location.status_flags & LEXER_LIT_LOCATION_HAS_ESCAPE)))
  {
    while ((literal_p = (lexer_lit_location_t *) parser_list_iterator_next (&literal_iterator)) != NULL)
    {
      if (literal_p->length == length)
                                ...
                                uint8_t stack_mode) /**< stack mode */
{
  ...
  JJS_ASSERT (!is_export_default || stack_mode == SCAN_STACK_CLASS_EXPRESSION);
#endif /* JJS_MODULE_SYSTEM */
...
{
  JJS_ASSERT (binding_type != SCANNER_BINDING_NONE || !is_nested);
...
scanner_pop_binding_list (parser_context_t *parser_context_p, scanner_context_t *scanner_context_p) /**< scanner context */
{
  ...
  JJS_ASSERT (prev_binding_list_p != NULL);
                           ...
                           parser_check_context_type_t check_type) /**< context type */
{
  ...
  JJS_ASSERT ((check_type == PARSER_CHECK_BLOCK_CONTEXT ? info_p->type == SCANNER_TYPE_BLOCK
                                                          : info_p->type == SCANNER_TYPE_FUNCTION));
  ...
  while (data_p[0] != SCANNER_STREAM_TYPE_END)
  {
    ...
    if (JJS_UNLIKELY (check_type == PARSER_CHECK_FUNCTION_CONTEXT))
    {
      if (JJS_UNLIKELY (type == SCANNER_STREAM_TYPE_HOLE))
      {
        data_p++;
        continue;
      }

      if (JJS_UNLIKELY (SCANNER_STREAM_TYPE_IS_ARGUMENTS (type)))
      {
    ...
    {
      JJS_ASSERT (type == SCANNER_STREAM_TYPE_VAR || type == SCANNER_STREAM_TYPE_LET
    ...
    {
      JJS_ASSERT (check_type == PARSER_CHECK_FUNCTION_CONTEXT);
...
{
  if (JJS_UNLIKELY (context_p->literal_count >= PARSER_MAXIMUM_NUMBER_OF_LITERALS))
  {
...
scanner_check_variables (parser_context_t *parser_context_p) /**< context */
{
  ...
  JJS_ASSERT (info_p->type == SCANNER_TYPE_FUNCTION);
  ...
  while (next_data_p[0] != SCANNER_STREAM_TYPE_END)
  {
    ...
    JJS_ASSERT (type != SCANNER_STREAM_TYPE_HOLE && !SCANNER_STREAM_TYPE_IS_ARG (type)
                          ...
                          uint32_t option_flags) /**< combination of scanner_create_variables_flags_t bits */
{
  ...
  JJS_ASSERT (info_type == SCANNER_TYPE_FUNCTION || info_type == SCANNER_TYPE_BLOCK);
  JJS_ASSERT (!(option_flags & SCANNER_CREATE_VARS_IS_FUNCTION_ARGS)
  ...
  while (next_data_p[0] != SCANNER_STREAM_TYPE_END)
  {
    ...
    JJS_ASSERT ((option_flags & (SCANNER_CREATE_VARS_IS_FUNCTION_BODY | SCANNER_CREATE_VARS_IS_FUNCTION_ARGS))
                  || (type != SCANNER_STREAM_TYPE_HOLE && !SCANNER_STREAM_TYPE_IS_ARG (type)
                      && !SCANNER_STREAM_TYPE_IS_ARG_FUNC (type)));

#if JJS_MODULE_SYSTEM
    JJS_ASSERT (type != SCANNER_STREAM_TYPE_IMPORT || (data_p[0] & SCANNER_STREAM_NO_REG));
#endif /* JJS_MODULE_SYSTEM */
      ...
      switch (type)
      {
        ...
        case SCANNER_STREAM_TYPE_VAR:
        {
          ...
          switch (type)
          {
            ...
            default:
            {
              JJS_ASSERT (type == SCANNER_STREAM_TYPE_LOCAL || type == SCANNER_STREAM_TYPE_DESTRUCTURED_ARG
                            || type == SCANNER_STREAM_TYPE_DESTRUCTURED_ARG_VAR
                            || type == SCANNER_STREAM_TYPE_DESTRUCTURED_ARG_FUNC);
 ...
 */
extern inline uint16_t JJS_ATTR_ALWAYS_INLINE
scanner_decode_map_to (parser_scope_stack_t *stack_item_p) /**< scope stack item */
{
  JJS_ASSERT (stack_item_p->map_from != PARSER_SCOPE_STACK_FUNC);
                            ...
                            uint16_t literal_index) /**< literal index */
{
  ...
  JJS_ASSERT ((scope_stack_p->map_to & PARSER_SCOPE_STACK_REGISTER_MASK) == 0);
