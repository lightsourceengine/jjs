                      ...
                      bool is_capturing) /**< is capturing group */
{
  ...
  if (JJS_UNLIKELY (!is_capturing && re_bytecode_size (re_ctx_p) == group_start_offset))
  {
    return;
  }
                               ...
                               bool negative) /** lookahead type */
{
  ...
  re_insert_byte (re_ctx_p, start_offset, (uint8_t) JJS_MIN (qmin, 1));
...
{
  JJS_ASSERT (re_ctx_p->input_curr_p < re_ctx_p->input_end_p);
  JJS_ASSERT (lit_char_is_octal_digit (*re_ctx_p->input_curr_p));
...
re_parse_char_class (re_compiler_ctx_t *re_ctx_p) /**< RegExp compiler context */
{
  ...
  while (true)
  {
    ...
    if (*re_ctx_p->input_curr_p == LIT_CHAR_BACKSLASH)
    {
      ...
      else
      {
        ...
        {
          JJS_ASSERT (re_ctx_p->token.type == RE_TOK_CHAR);
  ...
  JJS_ASSERT (escape_count <= RE_CLASS_ESCAPE_COUNT_MASK);
                      ...
                      bool expect_eof) /**< expect end of file */
{
  ...
  while (true)
  {
    ...
    JJS_ASSERT (ecma_is_value_empty (next_token_result));
