 ...
 */
ecma_value_t
opfunc_typeof (ecma_context_t *context_p, /**< JJS context */
               ecma_value_t left_value) /**< left value */
{
  return ecma_make_magic_string_value (ecma_get_typeof_lit_id (context_p, left_value));
} /* opfunc_typeof */
...
{
  JJS_ASSERT (!ecma_op_object_is_fast_array (object_p));

  ecma_property_t *property_p = ecma_find_named_property (context_p, object_p, prop_name_p);
  ecma_property_value_t *prop_value_p;

  if (property_p == NULL)
  {
    prop_value_p =
      ecma_create_named_data_property (context_p, object_p, prop_name_p, ECMA_PROPERTY_CONFIGURABLE_ENUMERABLE_WRITABLE, NULL);
  }
  else
  {
    JJS_ASSERT (ECMA_PROPERTY_IS_RAW (*property_p));
    ...
    if (!(*property_p & ECMA_PROPERTY_FLAG_DATA))
    {
      ...
      getter_setter_pair_p = ECMA_GET_NON_NULL_POINTER (context_p,
  ...
  ecma_named_data_property_assign_value (context_p, object_p, prop_value_p, value);
                     ...
                     ecma_value_t accessor) /**< accessor value */
{
  ...
  ecma_property_t *property_p = ecma_find_named_property (context_p, object_p, accessor_name_p);
  ecma_object_t *accessor_p = ecma_get_object_from_value (context_p, accessor);
  ...
  else
  {
...
#if JJS_CPOINTER_32_BIT
      ecma_getter_setter_pointers_t *getter_setter_pair_p;
      getter_setter_pair_p = jmem_heap_alloc_block (context_p, sizeof (ecma_getter_setter_pointers_t));
#endif /* JJS_CPOINTER_32_BIT */

      ecma_free_value_if_not_object (context_p, prop_value_p->value);
      *property_p = (uint8_t) (*property_p & ~(ECMA_PROPERTY_FLAG_DATA | ECMA_PROPERTY_FLAG_WRITABLE));

#if JJS_CPOINTER_32_BIT
      ECMA_SET_POINTER (context_p, getter_setter_pair_p->getter_cp, getter_func_p);
      ECMA_SET_POINTER (context_p, getter_setter_pair_p->setter_cp, setter_func_p);
      ECMA_SET_NON_NULL_POINTER (context_p, prop_value_p->getter_setter_pair_cp, getter_setter_pair_p);
#else /* !JJS_CPOINTER_32_BIT */
      ECMA_SET_POINTER (context_p, prop_value_p->getter_setter_pair.getter_cp, getter_func_p);
      ECMA_SET_POINTER (context_p, prop_value_p->getter_setter_pair.setter_cp, setter_func_p);
#endif /* JJS_CPOINTER_32_BIT */
      return;
    }
    ...
    if (is_getter)
    {
      ecma_set_named_accessor_property_getter (context_p, object_p, prop_value_p, accessor_p);
    }
    else
    {
      ecma_set_named_accessor_property_setter (context_p, object_p, prop_value_p, accessor_p);
    }
  }
} /* opfunc_set_accessor */
 ...
 */
ecma_value_t
vm_op_delete_prop (ecma_context_t *context_p, /**< JJS context */
                   ecma_value_t object, /**< base object */
                   ecma_value_t property, /**< property name */
                   bool is_strict) /**< strict mode */
{
  if (!ecma_op_require_object_coercible (context_p, object))
  {
    return ECMA_VALUE_ERROR;
  }

  ecma_string_t *name_string_p = ecma_op_to_property_key (context_p, property);
  ...
  ecma_value_t obj_value = ecma_op_to_object (context_p, object);
  /* The ecma_op_require_object_coercible call already checked the op_to_object error cases. */
  JJS_ASSERT (!ECMA_IS_VALUE_ERROR (obj_value));
  JJS_ASSERT (ecma_is_value_object (obj_value));
  ecma_object_t *obj_p = ecma_get_object_from_value (context_p, obj_value);
  JJS_ASSERT (!ecma_is_lexical_environment (obj_p));

  ecma_value_t delete_op_ret = ecma_op_object_delete (context_p, obj_p, name_string_p, is_strict);
  JJS_ASSERT (ecma_is_value_boolean (delete_op_ret) || ECMA_IS_VALUE_ERROR (delete_op_ret));
  ecma_deref_object (obj_p);
  ecma_deref_ecma_string (context_p, name_string_p);

  if (is_strict && ecma_is_value_false (delete_op_ret))
  {
    return ecma_raise_type_error (context_p, ECMA_ERR_OPERATOR_DELETE_RETURNED_FALSE_IN_STRICT_MODE);
  }
 ...
 */
ecma_value_t
vm_op_delete_var (ecma_context_t *context_p, /**< JJS context */
                  ecma_value_t name_literal, /**< name literal */
                  ecma_object_t *lex_env_p) /**< lexical environment */
{
  ecma_value_t completion_value = ECMA_VALUE_EMPTY;

  ecma_string_t *var_name_str_p = ecma_get_string_from_value (context_p, name_literal);

  ecma_object_t *ref_base_lex_env_p = ecma_op_resolve_reference_base (context_p, lex_env_p, var_name_str_p);

#if JJS_BUILTIN_PROXY
  if (JJS_UNLIKELY (ref_base_lex_env_p == ECMA_OBJECT_POINTER_ERROR))
  {
    return ECMA_VALUE_ERROR;
  }
  ...
  {
    JJS_ASSERT (ecma_is_lexical_environment (ref_base_lex_env_p));

    completion_value = ecma_op_delete_binding (context_p, ref_base_lex_env_p, var_name_str_p);
  }
 ...
 */
ecma_collection_t *
opfunc_for_in (ecma_context_t *context_p, /**< JJS context */
               ecma_value_t iterable_value, /**< ideally an iterable value */
               ecma_value_t *result_obj_p) /**< expression object */
{
  ...
  /* 4. */
  ecma_value_t obj_expr_value = ecma_op_to_object (context_p, iterable_value);
  /* ecma_op_to_object will only raise error on null/undefined values but those are handled above. */
  JJS_ASSERT (!ECMA_IS_VALUE_ERROR (obj_expr_value));
  ecma_object_t *obj_p = ecma_get_object_from_value (context_p, obj_expr_value);
  ecma_collection_t *prop_names_p = ecma_op_object_enumerate (context_p, obj_p);
  ...
  {
    *result_obj_p = ecma_make_object_value (context_p, obj_p);
    return prop_names_p;
  }

  ecma_deref_object (obj_p);
  ecma_collection_destroy (context_p, prop_names_p);
...
opfunc_append_to_spread_array (ecma_context_t *context_p, /**< JJS context */
                               ecma_value_t *stack_top_p, /**< current stack top */
                               uint16_t values_length) /**< number of elements to set */
{
  JJS_ASSERT (!(values_length & OPFUNC_HAS_SPREAD_ELEMENT));

  ecma_object_t *array_obj_p = ecma_get_object_from_value (context_p, stack_top_p[-1]);
  ...
  for (uint32_t i = 0, idx = old_length; i < values_length; i++, idx++)
  {
    ...
    if (stack_top_p[i] == ECMA_VALUE_SPREAD_ELEMENT)
    {
      ...
      ecma_value_t iterator = ecma_op_get_iterator (context_p, spread_value, ECMA_VALUE_SYNC_ITERATOR, &next_method);

      if (!ECMA_IS_VALUE_ERROR (iterator))
      {
        while (true)
        {
          ecma_value_t next_value = ecma_op_iterator_step (context_p, iterator, next_method);
          ...
          ecma_value_t value = ecma_op_iterator_value (context_p, next_value);
          ...
          JJS_ASSERT (ecma_is_value_true (put_comp));
      ...
      ecma_free_value (context_p, spread_value);

      if (ECMA_IS_VALUE_ERROR (ret_value))
      {
        for (uint32_t k = i + 1; k < values_length; k++)
        {
          ecma_free_value (context_p, stack_top_p[k]);
        }
    ...
    else
    {
      ecma_value_t put_comp = ecma_builtin_helper_def_prop_by_index (context_p,
                                                                     ...
                                                                     ECMA_PROPERTY_CONFIGURABLE_ENUMERABLE_WRITABLE);
      JJS_ASSERT (ecma_is_value_true (put_comp));
      ecma_free_value (context_p, stack_top_p[i]);
    }
  }
 ...
 */
JJS_ATTR_NOINLINE ecma_collection_t *
opfunc_spread_arguments (ecma_context_t *context_p, /**< JJS context */
                         ecma_value_t *stack_top_p, /**< pointer to the current stack top */
                         uint8_t arguments_list_len) /**< number of arguments */
{
  ecma_collection_t *buff_p = ecma_new_collection (context_p);
  ...
  for (uint32_t i = 0; i < arguments_list_len; i++)
  {
    ...
    {
      ecma_collection_push_back (context_p, buff_p, arg);
      continue;
    }
    ...
    if (!ECMA_IS_VALUE_ERROR (iterator))
    {
      while (true)
      {
        ...
        ecma_collection_push_back (context_p, buff_p, value);
      }
    }
    ...
    if (ECMA_IS_VALUE_ERROR (ret_value))
    {
      ...
      ecma_collection_free (context_p, buff_p);
      buff_p = NULL;
      break;
    }
  }
...
opfunc_append_array (ecma_context_t *context_p, /**< JJS context */
                     ecma_value_t *stack_top_p, /**< current stack top */
                     uint16_t values_length) /**< number of elements to set
                                              *   with potential OPFUNC_HAS_SPREAD_ELEMENT flag */
{
  ...
  {
    return opfunc_append_to_spread_array (context_p, stack_top_p, (uint16_t) (values_length & ~OPFUNC_HAS_SPREAD_ELEMENT));
  }
  ...
  if (JJS_LIKELY (ecma_op_array_is_fast_array (ext_array_obj_p)))
  {
    uint32_t filled_holes = 0;
    ecma_value_t *values_p = ecma_fast_array_extend (context_p, array_obj_p, old_length + values_length);
    ...
    for (uint32_t i = 0; i < values_length; i++)
    {
      ...
      if (!ecma_is_value_array_hole (stack_top_p[i]))
      {
        ...
        ecma_deref_if_object (context_p, stack_top_p[i]);
      }
    }

    ext_array_obj_p->u.array.length_prop_and_hole_count -= filled_holes * ECMA_FAST_ARRAY_HOLE_ONE;

    if (JJS_UNLIKELY ((values_length - filled_holes) > ECMA_FAST_ARRAY_MAX_NEW_HOLES_COUNT))
    {
      ecma_fast_array_convert_to_normal (context_p, array_obj_p);
    }
  }
  else
  {
    for (uint32_t i = 0; i < values_length; i++)
    {
      if (!ecma_is_value_array_hole (stack_top_p[i]))
      {
        ecma_string_t *index_str_p = ecma_new_ecma_string_from_uint32 (context_p, old_length + i);
                                 ...
                                 vm_create_executable_object_type_t type) /**< executable object type */
{
  ...
  size_t total_size = JJS_ALIGNUP (sizeof (vm_executable_object_t) + size, sizeof (uintptr_t));
  ...
  if (type == VM_CREATE_EXECUTABLE_OBJECT_GENERATOR)
  {
    ...
    proto_p = ecma_op_get_prototype_from_constructor (context_p, frame_ctx_p->shared_p->function_object_p, default_proto_id);
  }

  ecma_object_t *object_p = ecma_create_object (context_p, proto_p, total_size, ECMA_OBJECT_TYPE_CLASS);
  ...
  ECMA_SET_INTERNAL_VALUE_ANY_POINTER (context_p, executable_object_p->extended_object.u.cls.u3.head, NULL);
  executable_object_p->iterator = ECMA_VALUE_UNDEFINED;

  JJS_ASSERT (!(frame_ctx_p->status_flags & VM_FRAME_CTX_DIRECT_EVAL));
  ...
  *new_frame_ctx_p = *frame_ctx_p;
  ...
  {
    JJS_ASSERT (type != VM_CREATE_EXECUTABLE_OBJECT_GENERATOR);

    ecma_value_t *register_end_p = new_registers_p + register_end;

    JJS_ASSERT (register_end_p <= new_stack_top_p);

    while (new_registers_p < register_end_p)
    {
      ecma_deref_if_object (context_p, *new_registers_p++);
    }

    vm_ref_lex_env_chain (context_p, frame_ctx_p->lex_env_p, frame_ctx_p->context_depth, new_registers_p, false);

    new_registers_p += frame_ctx_p->context_depth;

    JJS_ASSERT (new_registers_p <= new_stack_top_p);
  }
  ...
  JJS_ASSERT (*VM_GET_EXECUTABLE_ITERATOR (new_frame_ctx_p) == ECMA_VALUE_UNDEFINED);

  new_frame_ctx_p->this_binding = ecma_copy_value_if_not_object (context_p, new_frame_ctx_p->this_binding);
                                 ...
                                 ecma_value_t value) /**< value pushed onto the stack (takes the reference) */
{
  ...
  if (executable_object_p->frame_ctx.context_depth > 0)
  {
    ...
    {
      ecma_ref_if_object (context_p, *register_p++);
    }

    vm_ref_lex_env_chain (context_p,
                          executable_object_p->frame_ctx.lex_env_p,
  ...
  ecma_ref_if_object (context_p, executable_object_p->iterator);

  JJS_ASSERT (ECMA_EXECUTABLE_OBJECT_IS_SUSPENDED (executable_object_p));
  ...
  context_p->global_object_p = ecma_op_function_get_realm (context_p, bytecode_header_p);
  ...
  {
    JJS_ASSERT (executable_object_p->frame_ctx.call_operation == VM_NO_EXEC_OP);
  ...
  if (executable_object_p->frame_ctx.context_depth > 0)
  {
    while (register_p < register_end_p)
    {
      ecma_deref_if_object (context_p, *register_p++);
    }
  ...
  ecma_deref_if_object (context_p, executable_object_p->iterator);
 ...
 */
void
opfunc_async_generator_yield (ecma_context_t *context_p, /**< JJS context */
                              ecma_extended_object_t *async_generator_object_p, /**< async generator object */
                              ecma_value_t value) /**< value (takes the reference) */
{
  ecma_async_generator_task_t *task_p;
  task_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_async_generator_task_t, async_generator_object_p->u.cls.u3.head);

  ecma_value_t iter_result = ecma_create_iter_result_object (context_p, value, ECMA_VALUE_FALSE);
  ecma_fulfill_promise (context_p, task_p->promise, iter_result);
  ...
  JJS_ASSERT (task_p->operation_value == ECMA_VALUE_UNDEFINED);
  jmem_heap_free_block (context_p, task_p, sizeof (ecma_async_generator_task_t));

  if (!ECMA_IS_INTERNAL_VALUE_NULL (next))
  {
    ecma_value_t executable_object = ecma_make_object_value (context_p, (ecma_object_t *) async_generator_object_p);
    ecma_enqueue_promise_async_generator_job (context_p, executable_object);
  }
                               ...
                               uint16_t extra_flags) /**< extra flags */
{
  ...
  ecma_promise_async_then (context_p, result, ecma_make_object_value (context_p, (ecma_object_t *) executable_object_p));
  ecma_deref_object ((ecma_object_t *) executable_object_p);
  ecma_free_value (context_p, result);

  result = ecma_op_create_promise_object (context_p, ECMA_VALUE_EMPTY, ECMA_VALUE_UNDEFINED, promise_p);

  JJS_ASSERT (ecma_is_value_object (result));
 ...
 */
static ecma_value_t
opfunc_private_method_or_accessor_add (ecma_context_t *context_p, /**< JJS context */
                                       ...
                                       uint32_t static_flag) /**< static_flag */
{
  ...
  ecma_value_t *collection_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_value_t, ECMA_PROPERTY_VALUE_PTR (prop_p)->value);
  ...
  while (current_p < end_p)
  {
    ...
    ecma_string_t *prop_name_p = ecma_get_symbol_from_value (context_p, *current_p++);
    ecma_value_t method = *current_p++;

    JJS_ASSERT (prop_name_p->u.hash & ECMA_SYMBOL_FLAG_PRIVATE_INSTANCE_METHOD);

    prop_p = ecma_find_named_property (context_p, this_obj_p, prop_name_p);
    ecma_object_t *method_p = ecma_get_object_from_value (context_p, method);

    if (kind == ECMA_PRIVATE_METHOD)
    {
      if (prop_p != NULL)
      {
        return ecma_raise_type_error (context_p, ECMA_ERR_CANNOT_DECLARE_SAME_PRIVATE_FIELD_TWICE);
      }

      ecma_property_value_t *prop_value_p =
        ecma_create_named_data_property (context_p, this_obj_p, prop_name_p, ECMA_PROPERTY_FIXED, NULL);
    ...
    if (prop_p == NULL)
    {
      ...
      ecma_create_named_accessor_property (context_p, this_obj_p, prop_name_p, getter_p, setter_p, ECMA_PROPERTY_FIXED, NULL);
      continue;
    }

    ecma_property_value_t *accessor_objs_p = ECMA_PROPERTY_VALUE_PTR (prop_p);
    ecma_getter_setter_pointers_t *get_set_pair_p = ecma_get_named_accessor_property (context_p, accessor_objs_p);

    if (kind == ECMA_PRIVATE_GETTER)
    {
      ECMA_SET_POINTER (context_p, get_set_pair_p->getter_cp, method_p);
    }
    else
    {
      JJS_ASSERT (kind == ECMA_PRIVATE_SETTER);
      ECMA_SET_POINTER (context_p, get_set_pair_p->setter_cp, method_p);
    }
  }
 ...
 */
ecma_value_t
opfunc_define_field (ecma_context_t *context_p, /**< JJS context */
                     ecma_value_t base, /**< base */
                     ecma_value_t property, /**< property */
                     ecma_value_t value) /**< value */
{
  ecma_string_t *property_key_p = ecma_op_to_property_key (context_p, property);

  JJS_ASSERT (property_key_p != NULL);
  ecma_object_t *obj_p = ecma_get_object_from_value (context_p, base);
  ecma_property_descriptor_t desc = ecma_make_empty_property_descriptor ();
  desc.value = value;
  desc.flags = (JJS_PROP_IS_WRITABLE | JJS_PROP_IS_WRITABLE_DEFINED | JJS_PROP_IS_ENUMERABLE
                | JJS_PROP_IS_ENUMERABLE_DEFINED | JJS_PROP_IS_CONFIGURABLE | JJS_PROP_IS_CONFIGURABLE_DEFINED
                | JJS_PROP_IS_VALUE_DEFINED | JJS_PROP_SHOULD_THROW);

  ecma_value_t result = ecma_op_object_define_own_property (context_p, obj_p, property_key_p, &desc);
  ecma_deref_ecma_string (context_p, property_key_p);
                          ...
                          ecma_value_t this_val) /**< this_arg of the function */
{
  ...
  ecma_object_t *this_obj_p = ecma_get_object_from_value (context_p, this_val);

  ecma_value_t result = opfunc_private_method_or_accessor_add (context_p, class_object_p, this_obj_p, 0);
  ...
  ecma_property_t *property_p = ecma_find_named_property (context_p, class_object_p, name_p);
  ...
  ecma_property_t *class_field_property_p = ecma_find_named_property (context_p, class_object_p, name_p);

  if (class_field_property_p != NULL)
  {
    ecma_value_t value = ECMA_PROPERTY_VALUE_PTR (class_field_property_p)->value;
    shared_class_fields.computed_class_fields_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_value_t, value);
  }

  ecma_property_value_t *property_value_p = ECMA_PROPERTY_VALUE_PTR (property_p);
  JJS_ASSERT (ecma_op_is_callable (context_p, property_value_p->value));

  ecma_extended_object_t *ext_function_p;
  ext_function_p = (ecma_extended_object_t *) ecma_get_object_from_value (context_p, property_value_p->value);
  shared_class_fields.header.bytecode_header_p = ecma_op_function_get_compiled_code (context_p, ext_function_p);
  shared_class_fields.header.function_object_p = &ext_function_p->object;

  ecma_object_t *scope_p =
    ECMA_GET_NON_NULL_POINTER_FROM_POINTER_TAG (context_p, ecma_object_t, ext_function_p->u.function.scope_cp);
...
opfunc_init_static_class_fields (ecma_context_t *context_p, /**< JJS context */
                                 ...
                                 ecma_value_t this_val) /**< this_arg of the function */
{
  JJS_ASSERT (ecma_op_is_callable (context_p, function_object));
  JJS_ASSERT (ecma_is_value_object (this_val));

  ecma_string_t *name_p = ecma_get_internal_string (LIT_INTERNAL_MAGIC_STRING_CLASS_FIELD_COMPUTED);
  ecma_object_t *function_object_p = ecma_get_object_from_value (context_p, function_object);
  ecma_property_t *class_field_property_p = ecma_find_named_property (context_p, function_object_p, name_p);
 ...
 */
ecma_value_t
opfunc_add_computed_field (ecma_context_t *context_p, /**< JJS context */
                           ecma_value_t class_object, /**< class object */
                           ecma_value_t name) /**< name of the property */
{
  ecma_string_t *prop_name_p = ecma_op_to_property_key (context_p, name);
  ...
  {
    name = ecma_make_symbol_value (context_p, prop_name_p);
  }
  else
  {
    name = ecma_make_string_value (context_p, prop_name_p);
  }

  ecma_string_t *name_p = ecma_get_internal_string (LIT_INTERNAL_MAGIC_STRING_CLASS_FIELD_COMPUTED);
  ecma_object_t *class_object_p = ecma_get_object_from_value (context_p, class_object);
  ...
  if (property_p == NULL)
  {
    ECMA_CREATE_INTERNAL_PROPERTY (context_p, class_object_p, name_p, property_p, property_value_p);
    compact_collection_p = ecma_new_compact_collection (context_p);
  }
  else
  {
    property_value_p = ECMA_PROPERTY_VALUE_PTR (property_p);
    compact_collection_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_value_t, property_value_p->value);
  }

  compact_collection_p = ecma_compact_collection_push_back (context_p, compact_collection_p, name);
 ...
 */
ecma_value_t
opfunc_create_implicit_class_constructor (ecma_context_t *context_p, /**< JJS context */
                                          ...
                                          const ecma_compiled_code_t *bytecode_p) /**< current byte code */
{
  ...
  prop_value_p = ecma_create_named_data_property (context_p,
                                                  ...
                                                  NULL);
  prop_value_p->value = ecma_make_uint32_value (context_p, 0);

  return ecma_make_object_value (context_p, function_object_p);
...
opfunc_set_home_object (ecma_context_t *context_p, /**< JJS context */
                        ecma_object_t *func_p, /**< function object */
                        ecma_object_t *parent_env_p) /**< parent environment */
{
  JJS_ASSERT (ecma_is_lexical_environment (parent_env_p));

  if (ecma_get_object_type (func_p) == ECMA_OBJECT_TYPE_FUNCTION)
  {
    ecma_extended_object_t *ext_func_p = (ecma_extended_object_t *) func_p;
    ECMA_SET_NON_NULL_POINTER_TAG (context_p,
 ...
 */
ecma_string_t *
opfunc_make_private_key (ecma_context_t *context_p, /**< JJS context */
                         ecma_value_t descriptor) /**< descriptor */
{
  ecma_string_t *private_key_p = ecma_new_symbol_from_descriptor_string (context_p, descriptor);
...
opfunc_find_private_key (ecma_context_t *context_p, /**< JJS context */
                         ecma_object_t *class_object_p, /**< class environment */
                         ...
                         ecma_string_t **out_private_key_p) /**< [out] private key */
{
  ...
  while (current_p < end_p)
  {
    ...
    {
      prop_p = ecma_find_named_property (context_p, obj_p, private_key_p);
                             ...
                             bool allow_heritage) /**< heritage flag */
{
  ...
  while (true)
  {
        ...
        && !ECMA_LEX_ENV_CLASS_IS_MODULE (lex_env_p))
    {
      ...
      ecma_property_t *prop_p = opfunc_find_private_key (context_p, class_object_p, obj_p, key_p, private_key_p);
                   ...
                   ecma_value_t property) /**< property */
{
  ...
  ecma_object_t *obj_p = ecma_get_object_from_value (context_p, base);
  ecma_string_t *prop_name_p = ecma_get_prop_name_from_value (context_p, property);
...
{
  ecma_object_t *obj_p = ecma_get_object_from_value (context_p, base);
  ecma_string_t *prop_name_p = ecma_get_string_from_value (context_p, property);
  ecma_string_t *private_key_p = NULL;
  ...
  ecma_property_value_t *value_p =
    ecma_create_named_data_property (context_p, obj_p, private_key_p, ECMA_PROPERTY_FLAG_WRITABLE, NULL);
...
opfunc_private_set (ecma_context_t *context_p, /**< JJS context */
                    ...
                    ecma_value_t value) /**< ecma value */
{
  ecma_value_t base_obj = ecma_op_to_object (context_p, base);
  ...
  ecma_object_t *obj_p = ecma_get_object_from_value (context_p, base_obj);
  ecma_string_t *prop_name_p = ecma_get_string_from_value (context_p, property);
  ecma_string_t *private_key_p = NULL;
  ...
  if (prop_p == NULL)
  {
    result = ecma_raise_type_error (context_p, ECMA_ERR_CANNOT_WRITE_PRIVATE_MEMBER_TO_AN_OBJECT_WHOSE_CLASS_DID_NOT_DECLARE_IT);
  }
  else if (*prop_p & ECMA_PROPERTY_FLAG_DATA)
  {
    ...
    {
      result = ecma_raise_type_error (context_p, ECMA_ERR_PRIVATE_METHOD_IS_NOT_WRITABLE);
    }
    else
    {
      ecma_value_assign_value (context_p, &ECMA_PROPERTY_VALUE_PTR (prop_p)->value, value);
      result = ecma_copy_value (context_p, value);
    }
  }
  else
  {
    ecma_getter_setter_pointers_t *get_set_pair_p = ecma_get_named_accessor_property (context_p, ECMA_PROPERTY_VALUE_PTR (prop_p));

    if (get_set_pair_p->setter_cp == JMEM_CP_NULL)
    {
      result = ecma_raise_type_error (context_p, ECMA_ERR_PRIVATE_FIELD_WAS_DEFINED_WITHOUT_A_SETTER);
    }
    else
    {
      ecma_object_t *setter_p = ECMA_GET_NON_NULL_POINTER (context_p, ecma_object_t, get_set_pair_p->setter_cp);

      result = ecma_op_function_call (context_p, setter_p, base, &value, 1);
    }
  }
                    ...
                    ecma_value_t property) /**< property name */
{
  ...
  if (prop_p == NULL)
  {
    result = ecma_raise_type_error (context_p, ECMA_ERR_CANNOT_READ_PRIVATE_MEMBER_TO_AN_OBJECT_WHOSE_CLASS_DID_NOT_DECLARE_IT);
  }
  else if (*prop_p & ECMA_PROPERTY_FLAG_DATA)
  {
    result = ecma_copy_value (context_p, ECMA_PROPERTY_VALUE_PTR (prop_p)->value);
  }
  else
  {
    ecma_getter_setter_pointers_t *get_set_pair_p = ecma_get_named_accessor_property (context_p, ECMA_PROPERTY_VALUE_PTR (prop_p));
    ...
    {
      result = ecma_raise_type_error (context_p, ECMA_ERR_PRIVATE_FIELD_WAS_DEFINED_WITHOUT_A_GETTER);
    }
    else
    {
      ecma_object_t *getter_p = ECMA_GET_NON_NULL_POINTER (context_p, ecma_object_t, get_set_pair_p->getter_cp);
      result = ecma_op_function_call (context_p, getter_p, base, NULL, 0);
    }
  }
...
opfunc_create_private_key (ecma_context_t *context_p, /**< JJS context */
                           ecma_value_t *collection_p, /**< collection of private properties */
                           ...
                           ecma_private_property_kind_t search_kind) /**< kind of the property */
{
  ...
  {
    return opfunc_make_private_key (context_p, search_key);
  }

  ecma_string_t *search_key_p = ecma_get_string_from_value (context_p, search_key);
 ...
 */
void
opfunc_collect_private_properties (ecma_context_t *context_p, /**< JJS context */
                                   ecma_value_t constructor, /**< constructor */
                                   ...
                                   uint8_t opcode) /**< opcode */
{
  ...
  JJS_ASSERT (ecma_is_value_object (constructor));
  JJS_ASSERT (ecma_is_value_string (prop_name));
  JJS_ASSERT (ecma_is_value_object (value) || ecma_is_value_undefined (value));

  ecma_object_t *constructor_p = ecma_get_object_from_value (context_p, constructor);
  ecma_string_t *internal_string_p = ecma_get_internal_string (LIT_INTERNAL_MAGIC_STRING_CLASS_PRIVATE_ELEMENTS);
  ecma_property_t *prop_p = ecma_find_named_property (context_p, constructor_p, internal_string_p);
  ecma_value_t *collection_p;
  ecma_property_value_t *prop_value_p;

  if (prop_p == NULL)
  {
    collection_p = ecma_new_compact_collection (context_p);
    ECMA_CREATE_INTERNAL_PROPERTY (context_p, constructor_p, internal_string_p, prop_p, prop_value_p);
    ECMA_SET_INTERNAL_VALUE_POINTER (context_p, prop_value_p->value, collection_p);
  }
  else
  {
    prop_value_p = ECMA_PROPERTY_VALUE_PTR (prop_p);
    collection_p = ECMA_GET_INTERNAL_VALUE_POINTER (context_p, ecma_value_t, prop_value_p->value);
  }

  ecma_string_t *key_p = opfunc_create_private_key (context_p, collection_p, prop_name, kind);
  ...
  collection_p = ecma_compact_collection_push_back (context_p, collection_p, (ecma_value_t) kind);
  collection_p = ecma_compact_collection_push_back (context_p, collection_p, ecma_make_symbol_value (context_p, key_p));
  collection_p = ecma_compact_collection_push_back (context_p, collection_p, value);

#ifndef JJS_NDEBUG
  ecma_value_t *end_p = ecma_compact_collection_end (collection_p);
  ecma_value_t *current_p = collection_p + 1;

  JJS_ASSERT ((end_p - current_p) % ECMA_PRIVATE_ELEMENT_LIST_SIZE == 0);
...
{
  JJS_ASSERT (ecma_is_value_string (class_name));
  ecma_context_t *context_p = frame_ctx_p->shared_p->context_p;
  ecma_object_t *class_env_p = ecma_create_decl_lex_env (context_p, frame_ctx_p->lex_env_p);

  /* 4.a */
  ecma_property_value_t *property_value_p;
  property_value_p = ecma_create_named_data_property (context_p,
                                                      class_env_p,
                                                      ecma_get_string_from_value (context_p, class_name),
                   ...
                   ecma_value_t *stack_top_p) /**< stack top */
{
  ...
  ecma_object_t *ctor_p = ecma_get_object_from_value (context_p, stack_top_p[-1]);
  ...
  {
    /* 6.f, 6.g.i */
    if (!ecma_is_constructor (context_p, super_class))
    {
      return ecma_raise_type_error (context_p, ECMA_ERR_CLASS_EXTENDS_NOT_CONSTRUCTOR);
    }

    ecma_object_t *parent_p = ecma_get_object_from_value (context_p, super_class);

    /* 6.g.ii */
    ecma_value_t proto_parent = ecma_op_object_get_by_magic_id (context_p, parent_p, LIT_MAGIC_STRING_PROTOTYPE);
    ...
    {
      proto_parent_p = ecma_get_object_from_value (context_p, proto_parent);
    ...
    {
      ecma_free_value (context_p, proto_parent);
      return ecma_raise_type_error (context_p, ECMA_ERR_PROPERTY_PROTOTYPE_IS_NOT_AN_OBJECT);
    }
  ...
  /* 7. */
  ecma_object_t *proto_p = ecma_create_object (context_p, proto_parent_p, 0, ECMA_OBJECT_TYPE_GENERAL);
  ecma_value_t proto = ecma_make_object_value (context_p, proto_p);

  ECMA_SET_POINTER (context_p, ctor_p->u2.prototype_cp, ctor_parent_p);

  if (free_proto_parent)
  {
    ecma_deref_object (proto_parent_p);
  }
  ecma_free_value (context_p, super_class);
  ...
  property_value_p = ecma_create_named_data_property (context_p,
                                                      ...
                                                      NULL);
  property_value_p->value = ecma_make_object_value (context_p, ctor_p);

  if (ecma_get_object_type (ctor_p) == ECMA_OBJECT_TYPE_FUNCTION)
  {
    opfunc_bind_class_environment (context_p, frame_ctx_p->lex_env_p, proto_p, ctor_p, ctor_p);
 ...
 */
ecma_object_t *
opfunc_bind_class_environment (ecma_context_t *context_p, /**< JJS context */
                               ecma_object_t *lex_env_p, /**< lexical environment */
                               ecma_object_t *home_object_p, /**< bound object */
                               ecma_object_t *ctor_p, /**< constructor object */
                               ecma_object_t *func_obj_p) /**< function object */
{
  ecma_object_t *proto_env_p = ecma_create_lex_env_class (context_p, lex_env_p, sizeof (ecma_lexical_environment_class_t));
  ECMA_SET_NON_NULL_POINTER (context_p, proto_env_p->u1.bound_object_cp, home_object_p);
  ...
  if (func_obj_p)
  {
    JJS_ASSERT (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_FUNCTION);
    ECMA_SET_NON_NULL_POINTER_TAG (context_p, ((ecma_extended_object_t *) func_obj_p)->u.function.scope_cp, proto_env_p, 0);
 ...
 */
static void
opfunc_set_class_attributes (ecma_context_t *context_p, /**< JJS context */
                             ecma_object_t *obj_p, /**< object */
                             ecma_object_t *parent_env_p) /**< parent environment */
{
  ...
  while (prop_iter_cp != JMEM_CP_NULL)
  {
    ...
    for (uint32_t index = 0; index < ECMA_PROPERTY_PAIR_ITEM_COUNT; index++)
    {
      ...
      {
        JJS_ASSERT (property == ECMA_PROPERTY_TYPE_DELETED
      ...
      if (property & ECMA_PROPERTY_FLAG_DATA)
      {
        if (ecma_is_value_object (property_pair_p->values[index].value) && ecma_is_property_enumerable (property))
        {
          ...
          opfunc_set_home_object (context_p, ecma_get_object_from_value (context_p, property_pair_p->values[index].value), parent_env_p);
        }
        continue;
      }
      ...
      if (get_set_pair_p->getter_cp != JMEM_CP_NULL)
      {
        opfunc_set_home_object (context_p, ECMA_GET_NON_NULL_POINTER (context_p, ecma_object_t, get_set_pair_p->getter_cp), parent_env_p);
      }

      if (get_set_pair_p->setter_cp != JMEM_CP_NULL)
      {
        opfunc_set_home_object (context_p, ECMA_GET_NON_NULL_POINTER (context_p, ecma_object_t, get_set_pair_p->setter_cp), parent_env_p);
      }
    }
 ...
 */
static void
opfunc_set_private_instance_method_attributes (ecma_context_t *context_p, /**< JJS context */
                                               ecma_object_t *class_object_p, /**< class constructor  */
                                               ecma_object_t *parent_env_p) /**< parent environment */
{
  ecma_string_t *internal_string_p = ecma_get_internal_string (LIT_INTERNAL_MAGIC_STRING_CLASS_PRIVATE_ELEMENTS);
  ecma_property_t *prop_p = ecma_find_named_property (context_p, class_object_p, internal_string_p);
  ...
  while (current_p < end_p)
  {
    ...
    {
      opfunc_set_home_object (context_p, ecma_get_object_from_value (context_p, value), parent_env_p);
    }
  }
...
{
  ecma_context_t *context_p = frame_ctx_p->shared_p->context_p;
  ecma_object_t *outer_env_p = ECMA_GET_NON_NULL_POINTER (context_p, ecma_object_t, frame_ctx_p->lex_env_p->u2.outer_reference_cp);
...
opfunc_finalize_class (vm_frame_ctx_t *frame_ctx_p, /**< frame context */
                       ...
                       ecma_value_t class_name) /**< class name */
{
  JJS_ASSERT (ecma_is_value_undefined (class_name) || ecma_is_value_string (class_name));
  ecma_context_t *context_p = frame_ctx_p->shared_p->context_p;
  ecma_value_t *stack_top_p = *vm_stack_top_p;
  ecma_object_t *ctor_p = ecma_get_object_from_value (context_p, stack_top_p[-2]);
  ecma_object_t *proto_p = ecma_get_object_from_value (context_p, stack_top_p[-1]);
  ...
  {
    ecma_op_initialize_binding (context_p, class_env_p, ecma_get_string_from_value (context_p, class_name), stack_top_p[-2]);
  }

  ecma_object_t *ctor_env_p = opfunc_bind_class_environment (context_p, class_env_p, ctor_p, ctor_p, NULL);
  ecma_object_t *proto_env_p = opfunc_bind_class_environment (context_p, class_env_p, proto_p, ctor_p, NULL);

  opfunc_set_class_attributes (context_p, ctor_p, ctor_env_p);
  opfunc_set_class_attributes (context_p, proto_p, proto_env_p);
  opfunc_set_private_instance_method_attributes (context_p, ctor_p, proto_env_p);

  ecma_deref_object (proto_env_p);
  ecma_deref_object (ctor_env_p);
  ecma_deref_object (proto_p);

  JJS_ASSERT ((ecma_is_value_undefined (class_name) ? stack_top_p[-3] == ECMA_VALUE_UNDEFINED
  ...
  ecma_value_t result = opfunc_private_method_or_accessor_add (context_p, ctor_p, ctor_p, ECMA_PRIVATE_PROPERTY_STATIC_FLAG);
                             ...
                             uint8_t opcode) /**< current cbc opcode */
{
  ...
  ecma_value_t parent = ecma_op_resolve_super_base (context_p, frame_ctx_p->lex_env_p);

  if (ECMA_IS_VALUE_ERROR (parent))
  {
    return ecma_raise_type_error (context_p, ECMA_ERR_INVOKE_NULLABLE_SUPER_METHOD);
  }

  if (!ecma_op_require_object_coercible (context_p, parent))
  {
    return ECMA_VALUE_ERROR;
  }
  ...
  {
    JJS_ASSERT (opcode == CBC_EXT_SUPER_PROP_ASSIGNMENT_REFERENCE
                  || opcode == CBC_EXT_SUPER_PROP_LITERAL_ASSIGNMENT_REFERENCE);
    *stack_top_p++ = parent;
    *stack_top_p++ = ecma_copy_value (context_p, prop_name);
    *vm_stack_top_p = stack_top_p;

    return ECMA_VALUE_EMPTY;
  }

  ecma_object_t *parent_p = ecma_get_object_from_value (context_p, parent);
  ecma_string_t *prop_name_p = ecma_op_to_property_key (context_p, prop_name);
  ...
  ecma_value_t result = ecma_op_object_get_with_receiver (context_p, parent_p, prop_name_p, frame_ctx_p->this_binding);
                               ...
                               uint32_t opcode_data) /**< opcode data to store the result */
{
  ...
  ecma_value_t base_obj = ecma_op_to_object (context_p, stack_top_p[-3]);
  ...
  ecma_object_t *base_obj_p = ecma_get_object_from_value (context_p, base_obj);
  ecma_string_t *prop_name_p = ecma_op_to_property_key (context_p, stack_top_p[-2]);
  ...
  ecma_value_t result =
    ecma_op_object_put_with_receiver (context_p, base_obj_p, prop_name_p, stack_top_p[-1], frame_ctx_p->this_binding, is_strict);
                             ...
                             ecma_value_t filter_array) /**< filter array */
{
  ...
  {
    source_object = ecma_op_to_object (context_p, source_object);
  ...
  ecma_object_t *source_object_p = ecma_get_object_from_value (context_p, source_object);
  ecma_collection_t *names_p = ecma_op_object_own_property_keys (context_p, source_object_p, JJS_PROPERTY_FILTER_ALL);

#if JJS_BUILTIN_PROXY
  if (names_p == NULL)
  {
    JJS_ASSERT (!source_to_object);
    return ECMA_VALUE_ERROR;
  }
#endif /* JJS_BUILTIN_PROXY */

  ecma_object_t *target_object_p = ecma_get_object_from_value (context_p, target_object);
  ...
  {
    ecma_object_t *filter_array_p = ecma_get_object_from_value (context_p, filter_array);

    JJS_ASSERT (ecma_get_object_type (filter_array_p) == ECMA_OBJECT_TYPE_ARRAY);
    JJS_ASSERT (ecma_op_object_is_fast_array (filter_array_p));

    if (filter_array_p->u1.property_list_cp != JMEM_CP_NULL)
    {
      filter_start_p = ECMA_GET_NON_NULL_POINTER (context_p, ecma_value_t, filter_array_p->u1.property_list_cp);
  ...
  {
    ecma_string_t *property_name_p = ecma_get_prop_name_from_value (context_p, *buffer_p++);
    ...
    if (filter_start_p != NULL)
    {
      ...
      do
      {
        if (ecma_compare_ecma_strings (property_name_p, ecma_get_prop_name_from_value (context_p, *filter_p)))
        {
          break;
        }
    ...
    result = ecma_op_object_get_own_property_descriptor (context_p, source_object_p, property_name_p, &descriptor);
    ...
    if (!(descriptor.flags & JJS_PROP_IS_ENUMERABLE))
    {
      ecma_free_property_descriptor (context_p, &descriptor);
      continue;
    }

    if ((descriptor.flags & JJS_PROP_IS_VALUE_DEFINED) && !ECMA_OBJECT_IS_PROXY (source_object_p))
    {
    ...
    else
    {
      ...
      result = ecma_op_object_get (context_p, source_object_p, property_name_p);
  ...
  if (JJS_UNLIKELY (source_to_object))
  {
                                             ...
                                             ecma_string_t *name_p) /**< binding name */
{
  ...
  ecma_property_t *binding_p = ecma_find_named_property (context_p, lex_env_p, name_p);
  ...
  ecma_object_t *global_obj_p = ecma_get_object_from_value (context_p, frame_ctx_p->this_binding);

#if JJS_BUILTIN_REALMS
  ecma_object_t *const global_scope_p = ecma_get_global_scope (context_p, (ecma_object_t *) context_p->global_object_p);
#else /* !JJS_BUILTIN_REALMS */
  ...
  if (ECMA_OBJECT_IS_PROXY (global_obj_p))
  {
    ...
    ecma_value_t status = ecma_proxy_object_get_own_property_descriptor (context_p, global_obj_p, name_p, &prop_desc);

    if (ecma_is_value_true (status))
    {
      status = ecma_make_boolean_value ((prop_desc.flags & JJS_PROP_IS_CONFIGURABLE) == 0);
  ...
  ecma_property_t property = ecma_op_object_get_own_property (context_p, global_obj_p, name_p, NULL, ECMA_PROPERTY_GET_NO_OPTIONS);

  JJS_ASSERT (property == ECMA_PROPERTY_TYPE_NOT_FOUND || ECMA_PROPERTY_IS_FOUND (property));
