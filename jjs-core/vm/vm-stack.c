...
JJS_STATIC_ASSERT (PARSER_WITH_CONTEXT_STACK_ALLOCATION == PARSER_BLOCK_CONTEXT_STACK_ALLOCATION,
                     with_context_stack_allocation_must_be_equal_to_block_context_stack_allocation);

JJS_STATIC_ASSERT (PARSER_WITH_CONTEXT_STACK_ALLOCATION == PARSER_TRY_CONTEXT_STACK_ALLOCATION,
                     with_context_stack_allocation_must_be_equal_to_block_context_stack_allocation);

JJS_STATIC_ASSERT (PARSER_FOR_OF_CONTEXT_STACK_ALLOCATION == PARSER_FOR_AWAIT_OF_CONTEXT_STACK_ALLOCATION,
                       ...
                       uint32_t search_limit) /**< search up-to this byte code */
{
  ...
  while (frame_ctx_p->context_depth > 0)
  {
    ...
    JJS_ASSERT (!VM_CONTEXT_IS_VARIABLE_LENGTH (context_type) || finally_type != VM_CONTEXT_FINALLY_JUMP);
    ...
    if (context_type == VM_CONTEXT_TRY || context_type == VM_CONTEXT_CATCH)
    {
      ...
      {
        JJS_ASSERT (byte_code_p[0] == CBC_EXT_OPCODE);
      ...
      {
        JJS_ASSERT (context_type == VM_CONTEXT_CATCH);
      ...
      if (JJS_UNLIKELY (byte_code_p[1] == CBC_EXT_ASYNC_EXIT))
      {
        branch_offset = (uint32_t) (byte_code_p - frame_ctx_p->byte_code_start_p);
      ...
      JJS_ASSERT (byte_code_p[1] >= CBC_EXT_FINALLY && byte_code_p[1] <= CBC_EXT_FINALLY_3);
...
vm_get_context_value_offsets (ecma_value_t *context_item_p) /**< any item of a context */
{
  switch (VM_GET_CONTEXT_TYPE (context_item_p[-1]))
  {
    ...
    default:
    {
      JJS_ASSERT (VM_GET_CONTEXT_TYPE (context_item_p[-1]) == VM_CONTEXT_FOR_OF
                      ...
                      bool do_ref) /**< ref or deref lexical environments */
{
  ...
  JJS_ASSERT (context_top_p > context_end_p);
