...
import logging
import time
import jjs_client_main

from jjs_client_websocket import WebSocket
from jjs_client_rawpacket import RawPacket
from jjs_client_tcp import Socket
...
def main():
    args = jjs_client_main.arguments_parse()
    ...
    if args.protocol == "tcp":
        ...
        from jjs_client_serial import Serial
    ...
    debugger = jjs_client_main.JJSDebugger(channel)
    debugger.non_interactive = args.non_interactive

    logging.debug("Connected to JJS")

    prompt = DebuggerPrompt(debugger)
    prompt.prompt = "(jjs-debugger) "
...
if __name__ == "__main__":
    try:
        ...
        MSG = str(error_msg)
        if ERRNO == 111:
            sys.exit("Failed to connect to the JJS debugger.")
