...
JJS_DEBUGGER_CONFIGURATION = 1
JJS_DEBUGGER_PARSE_ERROR = 2
JJS_DEBUGGER_BYTE_CODE_CP = 3
JJS_DEBUGGER_PARSE_FUNCTION = 4
JJS_DEBUGGER_BREAKPOINT_LIST = 5
JJS_DEBUGGER_BREAKPOINT_OFFSET_LIST = 6
JJS_DEBUGGER_SOURCE_CODE = 7
JJS_DEBUGGER_SOURCE_CODE_END = 8
JJS_DEBUGGER_SOURCE_CODE_NAME = 9
JJS_DEBUGGER_SOURCE_CODE_NAME_END = 10
JJS_DEBUGGER_FUNCTION_NAME = 11
JJS_DEBUGGER_FUNCTION_NAME_END = 12
JJS_DEBUGGER_WAITING_AFTER_PARSE = 13
JJS_DEBUGGER_RELEASE_BYTE_CODE_CP = 14
JJS_DEBUGGER_MEMSTATS_RECEIVE = 15
JJS_DEBUGGER_BREAKPOINT_HIT = 16
JJS_DEBUGGER_EXCEPTION_HIT = 17
JJS_DEBUGGER_EXCEPTION_STR = 18
JJS_DEBUGGER_EXCEPTION_STR_END = 19
JJS_DEBUGGER_BACKTRACE_TOTAL = 20
JJS_DEBUGGER_BACKTRACE = 21
JJS_DEBUGGER_BACKTRACE_END = 22
JJS_DEBUGGER_EVAL_RESULT = 23
JJS_DEBUGGER_EVAL_RESULT_END = 24
JJS_DEBUGGER_WAIT_FOR_SOURCE = 25
JJS_DEBUGGER_OUTPUT_RESULT = 26
JJS_DEBUGGER_OUTPUT_RESULT_END = 27
JJS_DEBUGGER_SCOPE_CHAIN = 28
JJS_DEBUGGER_SCOPE_CHAIN_END = 29
JJS_DEBUGGER_SCOPE_VARIABLES = 30
JJS_DEBUGGER_SCOPE_VARIABLES_END = 31
JJS_DEBUGGER_CLOSE_CONNECTION = 32

# Debugger option flags
JJS_DEBUGGER_LITTLE_ENDIAN = 0x1

# Subtypes of eval
JJS_DEBUGGER_EVAL_EVAL = "\0"
JJS_DEBUGGER_EVAL_THROW = "\1"
JJS_DEBUGGER_EVAL_ABORT = "\2"

# Subtypes of eval result
JJS_DEBUGGER_EVAL_OK = 1
JJS_DEBUGGER_EVAL_ERROR = 2

# Subtypes of output
JJS_DEBUGGER_OUTPUT_PRINT = 1
JJS_DEBUGGER_OUTPUT_LOG = 2

# Messages sent by the client to server.
JJS_DEBUGGER_FREE_BYTE_CODE_CP = 1
JJS_DEBUGGER_UPDATE_BREAKPOINT = 2
JJS_DEBUGGER_EXCEPTION_CONFIG = 3
JJS_DEBUGGER_PARSER_CONFIG = 4
JJS_DEBUGGER_MEMSTATS = 5
JJS_DEBUGGER_STOP = 6
JJS_DEBUGGER_PARSER_RESUME = 7
JJS_DEBUGGER_CLIENT_SOURCE = 8
JJS_DEBUGGER_CLIENT_SOURCE_PART = 9
JJS_DEBUGGER_NO_MORE_SOURCES = 10
JJS_DEBUGGER_CONTEXT_RESET = 11
JJS_DEBUGGER_CONTINUE = 12
JJS_DEBUGGER_STEP = 13
JJS_DEBUGGER_NEXT = 14
JJS_DEBUGGER_FINISH = 15
JJS_DEBUGGER_GET_BACKTRACE = 16
JJS_DEBUGGER_EVAL = 17
JJS_DEBUGGER_EVAL_PART = 18
JJS_DEBUGGER_GET_SCOPE_CHAIN = 19
JJS_DEBUGGER_GET_SCOPE_VARIABLES = 20

JJS_DEBUGGER_SCOPE_WITH = 1
JJS_DEBUGGER_SCOPE_LOCAL = 2
JJS_DEBUGGER_SCOPE_CLOSURE = 3
JJS_DEBUGGER_SCOPE_GLOBAL = 4
JJS_DEBUGGER_SCOPE_NON_CLOSURE = 5
...
def arguments_parse():
    parser = argparse.ArgumentParser(description="JJS debugger client")
...
class JJSPendingBreakpoint(object):
...
class JJSFunction(object):
    ...
    def __init__(self, is_func, byte_code_cp, source, source_name, line, column, name, lines, offsets):
        ...
        for i, _line in enumerate(lines):
            offset = offsets[i]
            breakpoint = JJSBreakpoint(_line, offset, self)
...
class JJSDebugger(object):
    ...
    def __init__(self, channel):
        ...
        if len(result) != config_size or ord(result[0]) != JJS_DEBUGGER_CONFIGURATION:
            raise Exception("Unexpected configuration")

        self.little_endian = ord(result[1]) & JJS_DEBUGGER_LITTLE_ENDIAN
        ...
        if self.version != JJS_DEBUGGER_VERSION:
            raise Exception("Incorrect debugger version from target: %d expected: %d" %
                            (self.version, JJS_DEBUGGER_VERSION))
    ...
    def quit(self):
        self.prompt = False
        self._exec_command(JJS_DEBUGGER_CONTINUE)
    ...
    def stop(self):
        self._exec_command(JJS_DEBUGGER_STOP)
    ...
    def next(self):
        self.prompt = False
        self._exec_command(JJS_DEBUGGER_NEXT)

    def step(self):
        self.prompt = False
        self._exec_command(JJS_DEBUGGER_STEP)
    ...
    def finish(self):
        self.prompt = False
        self._exec_command(JJS_DEBUGGER_FINISH)
    ...
    def eval(self, code):
        self._send_string(JJS_DEBUGGER_EVAL_EVAL + code, JJS_DEBUGGER_EVAL)
        self.prompt = False

    def eval_at(self, code, index):
        self._send_string(JJS_DEBUGGER_EVAL_EVAL + code, JJS_DEBUGGER_EVAL, index)
        self.prompt = False

    def throw(self, code):
        self._send_string(JJS_DEBUGGER_EVAL_THROW + code, JJS_DEBUGGER_EVAL)
        self.prompt = False

    def abort(self, args):
        self.delete("all")
        self.exception("0")  # disable the exception handler
        self._send_string(JJS_DEBUGGER_EVAL_ABORT + args, JJS_DEBUGGER_EVAL)
        self.prompt = False

    def restart(self):
        self._send_string(JJS_DEBUGGER_EVAL_ABORT + "\"r353t\"", JJS_DEBUGGER_EVAL)
        self.prompt = False
    ...
    def scope_chain(self):
        self.prompt = False
        self._exec_command(JJS_DEBUGGER_GET_SCOPE_CHAIN)
    ...
    def scope_variables(self, args):
        ...
        message = struct.pack(self.byte_order + "BB" + self.idx_format,
                              1 + 4,
                              JJS_DEBUGGER_GET_SCOPE_VARIABLES,
                              index)
    ...
    def memstats(self):
        self.prompt = False
        self._exec_command(JJS_DEBUGGER_MEMSTATS)
    ...
    def _send_string(self, args, message_type, index=0):
        ...
        if message_type == JJS_DEBUGGER_EVAL:
        ...
        if message_type == JJS_DEBUGGER_EVAL:
            message_type = JJS_DEBUGGER_EVAL_PART
        else:
            message_type = JJS_DEBUGGER_CLIENT_SOURCE_PART
    ...
    def _send_breakpoint(self, breakpoint):
        message = struct.pack(self.byte_order + "BBB" + self.cp_format + self.idx_format,
                              1 + 1 + self.cp_size + 4,
                              JJS_DEBUGGER_UPDATE_BREAKPOINT,
    ...
    def _send_bytecode_cp(self, byte_code_cp):
        message = struct.pack(self.byte_order + "BB" + self.cp_format,
                              1 + self.cp_size,
                              JJS_DEBUGGER_FREE_BYTE_CODE_CP,
                              byte_code_cp)
        self.channel.send_message(self.byte_order, message)
    ...
    def send_client_source(self):
        ...
        with open(path, 'r') as src_file:
            ...
            self._send_string(content, JJS_DEBUGGER_CLIENT_SOURCE)

    def send_no_more_source(self):
        self._exec_command(JJS_DEBUGGER_NO_MORE_SOURCES)
    ...
    def process_messages(self):
        ...
        while True:
            ...
            if buffer_type in [JJS_DEBUGGER_PARSE_ERROR,
                               JJS_DEBUGGER_BYTE_CODE_CP,
                               JJS_DEBUGGER_PARSE_FUNCTION,
                               JJS_DEBUGGER_BREAKPOINT_LIST,
                               JJS_DEBUGGER_SOURCE_CODE,
                               JJS_DEBUGGER_SOURCE_CODE_END,
                               JJS_DEBUGGER_SOURCE_CODE_NAME,
                               JJS_DEBUGGER_SOURCE_CODE_NAME_END,
                               JJS_DEBUGGER_FUNCTION_NAME,
                               JJS_DEBUGGER_FUNCTION_NAME_END]:
                result = self._parse_source(data)
                if result:
                    return DebuggerAction(DebuggerAction.TEXT, result)

            elif buffer_type == JJS_DEBUGGER_WAITING_AFTER_PARSE:
                self._exec_command(JJS_DEBUGGER_PARSER_RESUME)

            elif buffer_type == JJS_DEBUGGER_RELEASE_BYTE_CODE_CP:
                self._release_function(data)

            elif buffer_type in [JJS_DEBUGGER_BREAKPOINT_HIT, JJS_DEBUGGER_EXCEPTION_HIT]:
                ...
                if buffer_type == JJS_DEBUGGER_EXCEPTION_HIT:
            ...
            elif buffer_type == JJS_DEBUGGER_EXCEPTION_STR:
                self.exception_string += data[1:]

            elif buffer_type == JJS_DEBUGGER_EXCEPTION_STR_END:
                self.exception_string += data[1:]

            elif buffer_type == JJS_DEBUGGER_BACKTRACE_TOTAL:
                total = struct.unpack(self.byte_order + self.idx_format, data[1:])[0]
                result += "Total number of frames: %d\n" % (total)
                return DebuggerAction(DebuggerAction.TEXT, result)

            elif buffer_type in [JJS_DEBUGGER_BACKTRACE, JJS_DEBUGGER_BACKTRACE_END]:
                ...
                if buffer_type == JJS_DEBUGGER_BACKTRACE_END:
                    self.prompt = True
                else:
            ...
            elif buffer_type in [JJS_DEBUGGER_EVAL_RESULT,
                                 JJS_DEBUGGER_EVAL_RESULT_END,
                                 JJS_DEBUGGER_OUTPUT_RESULT,
                                 JJS_DEBUGGER_OUTPUT_RESULT_END]:

                result = self._process_incoming_text(buffer_type, data)
                return DebuggerAction(DebuggerAction.TEXT, result)

            elif buffer_type == JJS_DEBUGGER_MEMSTATS_RECEIVE:
            ...
            elif buffer_type == JJS_DEBUGGER_WAIT_FOR_SOURCE:
                self.send_client_source()

            elif buffer_type in [JJS_DEBUGGER_SCOPE_CHAIN, JJS_DEBUGGER_SCOPE_CHAIN_END]:
                self.scope_data = data[1:]

                if buffer_type == JJS_DEBUGGER_SCOPE_CHAIN_END:
            ...
            elif buffer_type in [JJS_DEBUGGER_SCOPE_VARIABLES, JJS_DEBUGGER_SCOPE_VARIABLES_END]:
                self.scope_vars += "".join(data[1:])

                if buffer_type == JJS_DEBUGGER_SCOPE_VARIABLES_END:
            ...
            elif JJS_DEBUGGER_CLOSE_CONNECTION:
                return DebuggerAction(DebuggerAction.END, "")
    ...
    def _parse_source(self, data):
        ...
        while True:
            ...
            if buffer_type == JJS_DEBUGGER_PARSE_ERROR:
                logging.error("Syntax error found")
                return ""

            elif buffer_type in [JJS_DEBUGGER_SOURCE_CODE, JJS_DEBUGGER_SOURCE_CODE_END]:
                source_code += data[1:]

            elif buffer_type in [JJS_DEBUGGER_SOURCE_CODE_NAME, JJS_DEBUGGER_SOURCE_CODE_NAME_END]:
                source_code_name += data[1:]

            elif buffer_type in [JJS_DEBUGGER_FUNCTION_NAME, JJS_DEBUGGER_FUNCTION_NAME_END]:
                function_name += data[1:]

            elif buffer_type == JJS_DEBUGGER_PARSE_FUNCTION:
            ...
            elif buffer_type in [JJS_DEBUGGER_BREAKPOINT_LIST, JJS_DEBUGGER_BREAKPOINT_OFFSET_LIST]:
                name = "lines"
                if buffer_type == JJS_DEBUGGER_BREAKPOINT_OFFSET_LIST:
                    name = "offsets"
            ...
            elif buffer_type == JJS_DEBUGGER_BYTE_CODE_CP:
            ...
            elif buffer_type in [JJS_DEBUGGER_OUTPUT_RESULT,
                                 JJS_DEBUGGER_OUTPUT_RESULT_END]:
                result += self._process_incoming_text(buffer_type, data)
    ...
    def _enable_breakpoint(self, breakpoint):
        if isinstance(breakpoint, JJSPendingBreakpoint):
    ...
    def _set_breakpoint(self, string, pending):
        ...
        if not result and not pending:
            ...
            if ans in ['yes\n', 'y\n']:
                ...
                if line:
                    breakpoint = JJSPendingBreakpoint(int(line.group(2)), line.group(1))
                else:
                    breakpoint = JJSPendingBreakpoint(function=string)
                result += self._enable_breakpoint(breakpoint)
    ...
    def _process_incoming_text(self, buffer_type, data):
        ...
        while True:
            if buffer_type in [JJS_DEBUGGER_EVAL_RESULT_END,
                               JJS_DEBUGGER_OUTPUT_RESULT_END]:
                subtype = ord(data[-1])
                message += data[1:-1]
                break
        ...
        # Subtypes of output
        if buffer_type == JJS_DEBUGGER_OUTPUT_RESULT_END:
            if subtype == JJS_DEBUGGER_OUTPUT_PRINT:
            ...
            if subtype == JJS_DEBUGGER_OUTPUT_LOG:
        ...
        if subtype == JJS_DEBUGGER_EVAL_ERROR:
    ...
    def _process_scope_variables(self):
        ...
        while buff_pos != buff_size:
            ...
            if value_type == JJS_DEBUGGER_VALUE_UNDEFINED:
                table.append([name, 'undefined', value])
            elif value_type == JJS_DEBUGGER_VALUE_NULL:
                table.append([name, 'Null', value])
            elif value_type == JJS_DEBUGGER_VALUE_BOOLEAN:
                table.append([name, 'Boolean', value])
            elif value_type == JJS_DEBUGGER_VALUE_NUMBER:
                table.append([name, 'Number', value])
            elif value_type == JJS_DEBUGGER_VALUE_STRING:
                table.append([name, 'String', value])
            elif value_type == JJS_DEBUGGER_VALUE_FUNCTION:
                table.append([name, 'Function', value])
            elif value_type == JJS_DEBUGGER_VALUE_ARRAY:
                table.append([name, 'Array', '[' + value + ']'])
            elif value_type == JJS_DEBUGGER_VALUE_OBJECT:
    ...
    def _process_scope(self):
        ...
        for i, level in enumerate(self.scope_data):
            if ord(level) == JJS_DEBUGGER_SCOPE_WITH:
                table.append([str(i), 'with'])
            elif ord(level) == JJS_DEBUGGER_SCOPE_GLOBAL:
                table.append([str(i), 'global'])
            elif ord(level) == JJS_DEBUGGER_SCOPE_NON_CLOSURE:
                # Currently it is only marks the catch closure.
                table.append([str(i), 'catch'])
            elif ord(level) == JJS_DEBUGGER_SCOPE_LOCAL:
                table.append([str(i), 'local'])
            elif ord(level) == JJS_DEBUGGER_SCOPE_CLOSURE:
