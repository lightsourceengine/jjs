 ...
 */
static jjs_value_t
jjsx_module_create_error (jjs_error_t error_type, /**< the type of error to create */
                            ...
                            const jjs_value_t module_name) /**< the module name */
{
  ...
  return jjs_throw_value (error_object, true);
} /* jjsx_module_create_error */

/**
 * Initialize the module manager extension.
 */
static void
jjsx_module_manager_init (void *user_data_p)
{
  *((jjs_value_t *) user_data_p) = jjs_object ();
} /* jjsx_module_manager_init */

/**
 * Deinitialize the module manager extension.
 */
static void
jjsx_module_manager_deinit (void *user_data_p) /**< context pointer to deinitialize */
{
  jjs_value_free (*(jjs_value_t *) user_data_p);
} /* jjsx_module_manager_deinit */

/**
 * Declare the context data manager for modules.
 */
static const jjs_context_data_manager_t jjsx_module_manager = { .init_cb = jjsx_module_manager_init,
                                                                    .deinit_cb = jjsx_module_manager_deinit,
                                                                    .bytes_needed = sizeof (jjs_value_t) };

/**
 * Global static entry point to the linked list of available modules.
 */
static jjsx_native_module_t *first_module_p = NULL;

void
jjsx_native_module_register (jjsx_native_module_t *module_p)
{
  module_p->next_p = first_module_p;
  first_module_p = module_p;
} /* jjsx_native_module_register */

void
jjsx_native_module_unregister (jjsx_native_module_t *module_p)
{
  jjsx_native_module_t *parent_p = NULL, *iter_p = NULL;
...
} /* jjsx_native_module_unregister */

/**
 * Attempt to retrieve a module by name from a cache, and return false if not found.
 */
static bool
jjsx_module_check_cache (jjs_value_t cache, /**< cache from which to attempt to retrieve the module by name */
                           jjs_value_t module_name, /**< JJS string value holding the module name */
                           jjs_value_t *result) /**< Resulting value */
{
  bool ret = false;

  /* Check if the cache has the module. */
  jjs_value_t js_has_property = jjs_object_has (cache, module_name);

  /* If we succeed in getting an answer, we examine the answer. */
  if (!jjs_value_is_exception (js_has_property))
  {
    bool has_property = jjs_value_is_true (js_has_property);
    ...
    if (has_property)
    {
      if (result != NULL)
      {
        (*result) = jjs_object_get (cache, module_name);
      }
      ret = true;
    }
  }

  jjs_value_free (js_has_property);

  return ret;
} /* jjsx_module_check_cache */
 ...
 */
static jjs_value_t
jjsx_module_add_to_cache (jjs_value_t cache, /**< cache to which to add the module */
                            jjs_value_t module_name, /**< key at which to cache the module */
                            jjs_value_t module) /**< the module to cache */
{
  jjs_value_t ret = jjs_object_set (cache, module_name, module);
 ...
 */
static bool
jjsx_resolve_native_module (const jjs_value_t canonical_name, /**< canonical name of the module */
                              jjs_value_t *result) /**< [out] where to put the resulting module instance */
{
  const jjsx_native_module_t *module_p = NULL;

  jjs_size_t name_size = jjs_string_size (canonical_name, JJS_ENCODING_UTF8);
  JJS_VLA (jjs_char_t, name_string, name_size);
  jjs_string_to_buffer (canonical_name, JJS_ENCODING_UTF8, name_string, name_size);
        ...
        && !strncmp ((char *) module_p->name_p, (char *) name_string, name_size))
    {
      ...
      (*result) =
        ((module_p->on_resolve_p) ? module_p->on_resolve_p ()
                                  : jjsx_module_create_error (JJS_ERROR_TYPE, on_resolve_absent, canonical_name));
      return true;
    }
  }

  return false;
} /* jjsx_resolve_native_module */

jjsx_module_resolver_t jjsx_module_native_resolver = { .get_canonical_name_p = NULL,
                                                           .resolve_p = jjsx_resolve_native_module };

static void
jjsx_module_resolve_local (const jjs_value_t name, /**< name of the module to load */
                             const jjsx_module_resolver_t **resolvers_p, /**< list of resolvers */
                             size_t resolver_count, /**< number of resolvers in @p resolvers */
                             jjs_value_t *result) /**< location to store the result, or NULL to remove the module */
{
  size_t index;
  size_t canonical_names_used = 0;
  jjs_value_t instances;
  JJS_VLA (jjs_value_t, canonical_names, resolver_count);
  jjs_value_t (*get_canonical_name_p) (const jjs_value_t name);
  bool (*resolve_p) (const jjs_value_t canonical_name, jjs_value_t *result);

  if (!jjs_value_is_string (name))
  {
    if (result != NULL)
    {
      *result = jjsx_module_create_error (JJS_ERROR_COMMON, module_name_not_string, name);
    }
    goto done;
  }

  instances = *(jjs_value_t *) jjs_context_data (&jjsx_module_manager);
  ...
  for (index = 0; index < resolver_count; index++)
  {
    ...
    canonical_names[index] = ((get_canonical_name_p == NULL) ? jjs_value_copy (name) : get_canonical_name_p (name));
    canonical_names_used++;
    if (jjsx_module_check_cache (instances, canonical_names[index], result))
    {
      /* A NULL for result indicates that we are to delete the module from the cache if found. Let's do that here.*/
      if (result == NULL)
      {
        jjs_object_delete (instances, canonical_names[index]);
      }
      goto done;
    }
  }
  ...
  for (index = 0; index < resolver_count; index++)
  {
    ...
    if (resolve_p != NULL && resolve_p (canonical_names[index], result))
    {
      ...
      {
        *result = jjsx_module_add_to_cache (instances, canonical_names[index], *result);
      }
      goto done;
    }
  }
  ...
  *result = jjsx_module_create_error (JJS_ERROR_COMMON, module_not_found, name);

done:
  /* Release the canonical names as returned by the various resolvers. */
  for (index = 0; index < canonical_names_used; index++)
  {
    jjs_value_free (canonical_names[index]);
  }
} /* jjsx_module_resolve_local */
 ...
 *
 * @return a jjs_value_t containing one of the followings:
 ...
 */
jjs_value_t
jjsx_module_resolve (const jjs_value_t name, /**< name of the module to load */
                       const jjsx_module_resolver_t **resolvers_p, /**< list of resolvers */
                       size_t resolver_count) /**< number of resolvers in @p resolvers */
{
  /* Set to zero to circumvent fatal warning. */
  jjs_value_t ret = 0;
  jjsx_module_resolve_local (name, resolvers_p, resolver_count, &ret);
  return ret;
} /* jjsx_module_resolve */

void
jjsx_module_clear_cache (const jjs_value_t name, /**< name of the module to remove, or undefined */
                           const jjsx_module_resolver_t **resolvers_p, /**< list of resolvers */
                           size_t resolver_count) /**< number of resolvers in @p resolvers */
{
  void *instances_p = jjs_context_data (&jjsx_module_manager);

  if (jjs_value_is_undefined (name))
  {
    /* We were requested to clear the entire cache, so we bounce the context data in the most agnostic way possible. */
    jjsx_module_manager.deinit_cb (instances_p);
    jjsx_module_manager.init_cb (instances_p);
    return;
  }

  /* Delete the requested module from the cache if it's there. */
  jjsx_module_resolve_local (name, resolvers_p, resolver_count, NULL);
