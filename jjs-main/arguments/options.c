             ...
             const char *opt) /**< optional part of the error message */
{
  if (!condition)
  {
    jjs_log (JJS_LOG_LEVEL_ERROR, "%s: %s%s\n", name, msg, opt != NULL ? opt : "");
    return false;
  }
  ...
  for (int id = cli_consume_option (&cli_state); id != CLI_OPT_END; id = cli_consume_option (&cli_state))
  {
    switch (id)
    {
      case OPT_HELP:
      {
        ...
        arguments_p->parse_result = JJS_STANDALONE_EXIT_CODE_OK;
        return false;
      }
      case OPT_VERSION:
      {
        printf ("Version: %d.%d.%d%s\n",
                JJS_API_MAJOR_VERSION,
                JJS_API_MINOR_VERSION,
                JJS_API_PATCH_VERSION,
                JJS_COMMIT_HASH);
      ...
      case OPT_MEM_STATS:
      {
        if (check_feature (JJS_FEATURE_HEAP_STATS, cli_state.arg))
        {
          ...
          arguments_p->init_flags |= JJS_CONTEXT_FLAG_MEM_STATS;
        }
        break;
      }
      case OPT_JJS_TEST:
      ...
      case OPT_SHOW_RE_OP:
      {
        ...
        {
          jjs_log_set_level (JJS_LOG_LEVEL_DEBUG);
          arguments_p->init_flags |= JJS_CONTEXT_FLAG_SHOW_REGEXP_OPCODES;
        }
        break;
      }
      case OPT_DEBUG_SERVER:
      {
        if (check_feature (JJS_FEATURE_DEBUGGER, cli_state.arg))
        {
      ...
      case OPT_EXEC_SNAP:
      {
        const bool is_enabled = check_feature (JJS_FEATURE_SNAPSHOT_EXEC, cli_state.arg);
      ...
      case OPT_LOG_LEVEL:
      {
        ...
        jjs_log_set_level ((jjs_log_level_t) log_level);
        break;
      }
      case OPT_NO_PROMPT:
      {
